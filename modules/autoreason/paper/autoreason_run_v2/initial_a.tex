...
\textbf{Limitations.}\enspace All results from one model family (Anthropic). All evaluation relies on LLM judges rather than human raters; while we mitigate known biases through randomization, blind evaluation, and CoT reasoning, LLM-as-judge remains an imperfect proxy for human quality assessment, and our conclusions about output quality are bounded by this limitation. No human evaluation was conducted. The constrained-task result demonstrating scope as the root cause comes from a single task (a 500-word startup pitch); while the finding is consistent with the diagnosis from eight remedy experiments, generalization to other constrained task types remains untested. The paper-writing experiment used Opus, not Sonnet. Scaling remedy experiments each ran once; Monte Carlo replication would strengthen the claims.
...
\bibitem{arrow} K. Arrow. \textit{Social Choice and Individual Values}. Wiley, 1951.\footnote{References~\cite{slopcodebench} and~\cite{ace} carry 2026 dates; these are correct and reflect recent preprints available at the time of submission.}
\end{thebibliography}
