...
\textbf{Compute budget.}\enspace Autoreason requires ${\sim}$5 LLM calls per pass (critic, author~B, synthesizer, 3 judges) and typically runs 10--25 passes to convergence (${\sim}$50--125 calls per task), while each baseline requires ${\sim}$1 call per pass (${\sim}$15 calls total). This ${\sim}$6$\times$ compute difference is a real cost of the method; autoreason trades compute for evaluation quality. We use 3 judges in-loop to balance per-pass cost against evaluation stability, and 7 judges for final comparisons to increase statistical power where cost is amortized over a single evaluation.
...
\caption{The scaling story. Same model (Sonnet~4.6), same method, opposite results. Left: unconstrained task, autoreason last. Right: scope-constrained task, autoreason first.}
