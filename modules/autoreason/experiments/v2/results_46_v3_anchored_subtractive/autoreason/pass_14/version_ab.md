...
6. **No timeout on Redis reads — a hung read blocks `publish_once` indefinitely** — `redis.Redis` is now instantiated with `socket_timeout` and `socket_connect_timeout`. A hung Redis read raises after the timeout, `publish_once` fails fast, and the health file goes stale on the normal staleness schedule.
...
8. **AUTH cap failure mode is unaddressed — no fallback when AUTH SMS is exhausted** — Section 2.6 now specifies the AUTH SMS fallback: when the AUTH sub-cap is within 20% of exhaustion, the system switches to in-app notification with a polling fallback for OTP delivery. The complete failure mode is stated plainly.
...
#   = 60 + 10 + 30 = 100s
#
# We allow for two consecutive failed cycles before declaring the publisher
# unhealthy. Two cycles = 2 × 100s = 200s. We use 150s as the staleness
# threshold, accepting that a publisher failing consistently on every cycle
# will be detected within ~2.5 minutes.
#
# Tradeoff: a shorter threshold (e.g., 90s) would catch failures faster but
# would cause false-positive restarts if CloudWatch is briefly slow. 150s
# requires CloudWatch to be slow for two full consecutive cycles to produce
# a false positive — unlikely in practice.
...
try:
    ...
    if age > STALENESS_THRESHOLD_SECONDS:
        print(f"Health file stale: {age:.0f}s > {STALENESS_THRESHOLD_SECONDS}s")
        sys.exit(1)
    sys.exit(0)
...
# rather than blocking publish_once indefinitely.
...
def _get_current_cap(channel: str) -> tuple[int, bool]:
    ...
    In both failure cases, the caller emits CapReadHealthy=0.
    The circuit breaker is not affected — it reads Redis directly.
    """
    try:
        ...
        if cached is not None:
            logger.warning("Using cached cap value %d for %s", cached, channel)
            return cached, False
        logger.error("No cached cap value for %s — emitting sentinel", channel)
        return _CAP_SENTINEL, False
