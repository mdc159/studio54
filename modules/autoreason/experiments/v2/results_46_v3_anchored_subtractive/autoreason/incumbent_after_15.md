...
1. **Staleness threshold math is still wrong — 150s is less than one worst-case cycle plus one publish interval** — The threshold is corrected to 220s. The derivation is shown step by step. The prose claim ("two consecutive failed cycles") now matches the number.

2. **`publish_once` is cut off mid-function — health file write timing is unspecified** — The complete `publish_once` function is now shown. The health file is written only after CloudWatch confirms delivery. A partial success (Redis read succeeded, CloudWatch failed) does not update the health file. The operational consequence of this choice is stated explicitly.
...
4. **The "atomic" conditional write guarantee is false if AUTH and SOCIAL caps are separate DynamoDB items** — The table schema is now shown. AUTH and SOCIAL consumed counters are stored as attributes on a single DynamoDB item, not separate items. A single conditional write can atomically check and update both values. The atomicity claim is now grounded in the actual schema.

5. **Sections 2.4 through 2.9 are missing — five of eight claimed resolutions are in sections that don't exist** — All sections are now present in full: Section 2.4 (CloudWatch alarm definitions), Section 2.5 (Lambda code and DLQ), Section 2.6 (AUTH SMS fallback), Section 2.7 (runbook RB-06), Section 2.8 (cap constraint enforcement), Section 2.9 (cap configuration change procedure).

6. **`_CAP_SENTINEL = -1` causes undefined behavior in ratio alarms** — The sentinel value is removed. When DynamoDB fails with no cached value, the publisher emits `CapReadHealthy=0` and skips emitting a cap metric entirely for that cycle. The consumption alarm uses a separate metric that is unaffected. The alarm on `INSUFFICIENT_DATA` state handles the missing cap metric.

7. **E1 owns all three AUTH cap review checkpoints with no backup or escalation path** — Each checkpoint now has a primary and a secondary owner. A missed review has a defined escalation path: the secondary owner takes over, and if both are unavailable, the review is rescheduled within five business days with the engineering manager notified.

8. **The Twilio baseline is established pre-launch against pre-launch data** — The pre-launch query is now explicitly labeled a provisional baseline. A post-launch baseline is established at the end of week 2 of production operation. The month-1 checkpoint compares post-launch data against the post-launch baseline, not the pre-launch estimate. The pre-launch query's purpose is now stated correctly: it sets the initial cap before any post-launch data exists, not the permanent baseline.
...
**This query produces a provisional baseline only.** The pre-launch auth service operates without the new notification system. Once the notification system is live and driving increased user engagement, AUTH SMS volume may change — more active users means more login events, more OTP requests, more security alerts. The pre-launch query cannot predict post-launch volume. Its purpose is to set the initial cap before any post-launch data exists, not to establish the permanent operating baseline.

The permanent baseline is established at the end of week 2 of production operation, once the system has stabilized and the pre-launch traffic spike has subsided. The month-1 checkpoint (Section 2.2, cap review schedule) compares post-launch data against the post-launch baseline, not the pre-launch estimate.

The 20K initial cap is set to ≥ 3× the observed pre-launch p95 (4,800/day). This provides margin for the baseline shift described above. If the post-launch week-2 baseline shows p95 above 6,600/day, the AUTH cap is raised before the month-1 checkpoint and the SOCIAL cap is reduced proportionally, subject to the total constraint.

**What the 3× headroom does and does not protect against:**

3× headroom over the observed p95 means the cap tolerates AUTH volume growing to 20K/day before exhaustion. At the observed pre-launch p95 of 4,800/day, this is a 4× growth margin. However, when the AUTH cap is exhausted, AUTH SMS fails completely — OTP delivery via SMS stops. The fallback for this condition is specified in Section 2.6. The headroom rationale is about deferring the need for manual intervention; it does not eliminate the consequence of exhaustion.
...
**Missed review procedure:** If the primary owner cannot complete a review, the secondary owner completes it within two business days of the scheduled date. If both are unavailable, the engineering manager is notified immediately and the review is rescheduled within five business days. Reviews are not silently deferred. The review completion date and result are recorded in the team wiki regardless of which owner performs the review.

**Review action threshold:** If the review finds that the observed p95 has grown more than 20% from the previous checkpoint's baseline, the primary owner raises a ticket to adjust the AUTH cap before the next checkpoint. The ticket is assigned to E1 and tracked in the sprint board.
...
The circuit breaker reads Redis directly to check cumulative consumption against the cap. This is the mechanism that gates SMS dispatch. The publisher's health is independent of the circuit breaker's ability to read Redis — but Redis availability is not independent. If Redis is unreachable, both the publisher and the circuit breaker lose their data source simultaneously.
...
# The circuit breaker and the publisher use the same timeout values
# but are separate processes with separate Redis connections.
...
#   = CloudWatch call timeout (not specified in constants; use 30s AWS SDK default)
#     × MAX_RET