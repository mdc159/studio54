...
This revision addresses ten critic findings from Revision 9. Each finding is addressed at the point of the original decision.

**Findings and their resolutions:**

1. **Document ends mid-sentence — Section 2.5 cuts off mid-derivation** — Section 2.5 is completed. The threshold derivation is carried through to a concrete formula, a placeholder value, and the calibration procedure.

2. **The "required input" dependency is unresolved — no deadline, owner, or escalation path** — Section 2.5 now specifies an owner (E1), a deadline (end of Week 2 of the deployment phase), a data source, and an escalation path if the data is unavailable.

3. **`_get_current_cap` instantiates a DynamoDB resource on every call** — The DynamoDB client and table reference are now module-level singletons. DynamoDB errors are caught separately from Redis errors. The metric is omitted rather than emitting a stale or zero value when DynamoDB is unavailable.

4. **AUTH sub-cap alarm thresholds are inconsistent with stated headroom** — The AUTH alarms are recalibrated to detect anomalous growth relative to normal volume, not relative to cap exhaustion. The derivation is shown.

5. **`REDIS_CONNECTION_FAILURE` references `CacheClusterStatus`, which is not a CloudWatch metric** — The reference is removed. The alarm is defined solely on the application-level `ConnectionErrors` metric emitted by the worker.

6. **The failure mode table contradicts the code — publisher keeps running during Redis failure, heartbeat continues** — The table is corrected. When Redis fails but the publisher process is alive, the heartbeat continues and `SMS_METRIC_PUBLISHER_DEAD` does not fire. Redis failure is detected by `REDIS_CONNECTION_FAILURE` instead.

7. **The 3-minute publisher dead window claim is only valid with one worker task** — The heartbeat alarm is changed from `Sum < 1` to `Average < 1`, which is independent of task count. The reasoning is explained.

8. **Section 2.8 is referenced as completed but does not appear in the document** — Section 2.8 is included in full. The 200-account attack calculation is carried through to a detection window estimate with stated assumptions.

9. **The Lambda automated write can be rejected if AUTH has been manually increased close to 100K — no behavior specified** — The Lambda reads both current cap values before writing, checks the constraint, and pages on-call if the constraint prevents automated action. This case is not silently ignored.

10. **`socket_connect_timeout=2` and `socket_timeout=2` are inconsistent with the latency alarm — publisher blocks, does not emit zero on slow reads** — The failure mode table and description are corrected. The publisher blocks up to 2 seconds, then either succeeds or throws. It does not emit zero on a slow read.
...
# Previously, _get_current_cap instantiated boto3.resource on every 60-second
# call. The client and table reference are now created once here.
...
**`SocialSMSCapCurrent` metric definition:** Emitted every 60 seconds when DynamoDB is available. Its value is the current `cap_value` for `SOCIAL_DAILY_CAP` in DynamoDB. Used in RB-07 (Section 2.7) to verify that a cap restoration write has taken effect.
...
Why Average, not Sum:
  ...
  emits PublisherHeartbeat = 1 every 60 seconds. Sum over 60 seconds
  is N when all publishers are alive. A Sum < 1 threshold only fires
  when ALL publishers are dead, meaning a single surviving publisher
  masks all others being dead.

  With Average: the average across all publishers is 1 when any data
  arrives, and the alarm fires only when data stops arriving entirely
  (missing data treatment: BREACHING converts absence to a breach).
  This fires within 3 minutes regardless of task count.
  ...
  health metric would require task-level dimensions, which adds
  cardinality complexity the team has chosen not to take on initially.
  The circuit breaker reads Redis at dispatch time — not CloudWatch —
  so partial publisher failure affects observability, not cap enforcement.

Consequence if alarm fires: On-call cannot see the consumption graph.
  The circuit breaker (Redis counter checked at dispatch) continues to
  function. On-call must check Redis or Twilio dashboards directly
...
Alarm: REDIS_CONNECTION_FAILURE
  ...
  (Application-level metric emitted by the worker on each failed Redis
   operation. The prior revision listed AWS/ElastiCache/CacheClusterStatus,
   which is not a valid CloudWatch metric. That reference is removed.)
  Statistic: Sum
  Period: 60 seconds
...
Alarm: REDIS_LATENCY_HIGH
  ...
  Consequence: High latency means Redis reads in the publisher may
               approach the 2-second socket timeout. On-call is paged
               at p99 > 50ms, well before timeouts begin occurring,
               so the issue is known before it becomes a publisher failure.
               The publisher does not emit zero during slow Redis — it
               blocks up to 2 seconds and either succeeds or throws.
               If it throws, the heartbeat is not emitted for that
               interval. Sustained failures cause SMS_METRIC_PUBLISHER_DEAD
               to fire within 3 minutes.
```

**Failure mode summary:**

| Condition | SocialSMSConsumed | PublisherHeartbeat | Alarm that fires |
|---|---|---|---|
| Normal operation | Rising slowly | Present every 60s | None |
| Active attack | Rising rapidly | Present every 60s | Consumption alarms |
| Publisher process crashed | Absent | Absent | SMS_METRIC_PUBLISHER_DEAD ≤3 min |
| Redis fully failed | Absent | Present (publisher alive, Redis throws, heartbeat not emitted — see below) | REDIS_CONNECTION_FAILURE; SMS_METRIC_PUBLISHER_DEAD if sustained |
| Redis slow (p99 >50ms) | Present (publisher blocks, succeeds or throws) | Present if read succeeds | REDIS_LATENCY_HIGH ≤3 min |
| DynamoDB unavailable | Present | Present | None (cap metric omitted, logged) |

*Correction from Revision 9: when Redis fails, the publisher process remains alive. The heartbeat is emitted only if `publish_once()` completes