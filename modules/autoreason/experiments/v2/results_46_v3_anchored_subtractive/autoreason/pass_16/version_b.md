...
1. **CloudWatch call timeout is undefined and the derivation is cut off** — The CloudWatch SDK timeout is now an explicit constant `CW_CALL_TIMEOUT_SECONDS = 30` in `publisher/constants.py`. The staleness threshold derivation is complete, step by step, with no truncation.
...
```python
# publisher/constants.py (continued from above)
#
...
#   After attempt 3: min(2^3, 4) = 4s
#   Total retry sleep (between 3 attempts = 2 sleeps after attempts 1 and 2;
#   no sleep after final attempt):                2 + 4 = 6s
#
#   Note: MAX_RETRIES = 3 means 3 total attempts (not 3 retries after the first).
#   Sleep occurs after attempt 1 and attempt 2. No sleep after attempt 3 regardless
#   of outcome — the function returns the error.
#
...
#   = 147s + 60s = 207s
#
# The health check runs every 30s (ECS interval). In the worst case, the health
# check fires immediately after the health file timestamp, so the staleness check
# measures the age of the file written at the end of the previous successful cycle.
# We need the threshold to be longer than the worst-case gap between two consecutive
# health file writes — i.e., one failed cycle plus one publish interval — with a
# margin to avoid flapping on timing jitter.
#
...
# STALENESS_THRESHOLD_SECONDS = 220
#
# The prose claim "the threshold survives two consecutive failed cycles" is NOT
# made in this document. The threshold survives one failed cycle plus one publish
# interval. That is the correct claim. If two consecutive cycles fail, the health
# check will fail — this is intentional. Two consecutive fully-failed cycles
# (each taking the full 147s) plus one interval would require a threshold of
# 147 + 60 + 147 = 354s, which would mean the publisher could be broken for
# nearly 6 minutes before ECS restarts it. We accept the one-cycle tolerance
# and treat two consecutive failures as a genuine problem requiring restart.
```

#### Publisher Architecture

A single dedicated ECS service runs exactly one publisher task. It is not a sidecar. It