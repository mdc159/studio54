...
This revision addresses ten problems identified in the prior review. The table below records each problem, the resolution adopted, and its location.

| # | Problem | Resolution | Location |
|---|---------|------------|----------|
| 1 | Document cut off mid-sentence; step 4 of fill sequence incomplete | Step 4 completed; all conditions the NSTL must verify before confirming those rows are written in full | §0.3 |
| 2 | Revision table claims Gate 4 written in full but §0.4 absent from submitted document | Gates 0–5 and §0.4 written in full in this document; Gate 4 includes Configuration B definition and tiebreak procedure | §0.4 |
| 3 | 13-hour monthly threshold conceals a real spike in Month 2 when Gate 3 event plus normal duties may reach 9.5–10.5 hours without breaching the threshold | Threshold restructured: a separate single-month spike threshold of 8 hours applies to any individual month; the 13-hour figure is retired; Month 2 spike is explicitly called out as a monitoring point | §0.2 |
| 4 | Proxy confirmation path has circular dependency: gate owner who cannot access repository and has not received automated notification has no reliable way to know a commit was made | NSTL is assigned affirmative duty to attempt direct contact with gate owner within 4 hours of proxy commit, independent of whether automated notification fires; gate owner awareness does not depend on the automated notification system | §0.1 |
| 5 | NSTL Backup activation by any team member under simultaneous EM and NSTL non-performance is unverifiable; no procedure for the backup to confirm legitimacy | Activation by a non-NSTL, non-EM team member requires a written statement of observed facts sent to the NSTL Backup and copied to the oversight body contact stored in the tracker; NSTL Backup confirms activation is not contested within 4 business hours before assuming duties | §0.2 |
| 6 | Steps 2 and 3 of fill sequence both require countersigning the Deputy EM row; no mechanism resolves a discrepancy between the two oversight body signers | Steps 2 and 3 merged into a single step requiring two oversight body signatures on the same row simultaneously; discrepancy procedure written | §0.3 |
| 7 | "Second oversight body member" not defined; EM could informally influence which member is selected | Selection procedure specified: the NSTL requests the confirming member by contacting the oversight body's general inbox directly, without routing through the EM; the oversight body assigns the confirming member independently | §0.3 |
| 8 | Residual limitation on HR system escalation path has no documented endpoint if general inbox also fails | Escalation endpoint documented: if general inbox produces no valid representative within two business days, any team member may escalate to the organization's CEO or equivalent executive authority, whose contact is obtained from the same HR system | §0.2 |
| 9 | Return of duties after NSTL absence requires EM acknowledgment; EM non-performance at that moment creates ambiguous dual-holding state | EM acknowledgment replaced with a documented handoff entry in the tracker signed by both NSTL and NSTL Backup; if EM is non-performing, the handoff is valid without EM acknowledgment and the Deputy EM is notified | §0.2 |
| 10 | Overrun threshold based entirely on self-reported time log; no standard for implausibility defined in EM spot-check | EM spot-check given three defined implausibility tests; secondary cross-check against observable proxy events and escalation records introduced as independent corroboration | §0.2 |
...
3. **Automated notification:** The repository platform's built-in commit notification emails all individuals whose addresses are registered in the platform's member list for this repository. Registration is performed by the Engineering Manager at project kickoff: each person listed in §0.3 is added to the repository as a member with their organizational email address before Gate 0 resolves. The NSTL confirms in the tracker that all addresses are registered before Gate 0 can resolve. Upon the NSTL's commit in step 2, the platform sends an automated notification to the gate owner at their registered organizational email address.
4. **NSTL affirmative contact — independent of automated notification:** Within 4 hours of the proxy commit in step 2, the NSTL must also contact the gate owner directly by email (not relying on the automated notification) stating: the commit was made, the commit hash, and the 48-hour confirmation window. This contact is logged in the tracker with a timestamp. The gate owner's obligation to confirm within 48 hours begins from the timestamp of this direct NSTL contact, not from receipt of the automated notification. The automated notification in step 3 is supplementary; the NSTL's direct contact is the operative trigger. This resolves the prior circular dependency: the gate owner's awareness of the proxy commit does not depend on the automated system functioning.
...
8. If the gate owner does not confirm within the window, the commit remains `[proxy-provisional]`. A `[proxy-provisional]` resolution does not unblock dependent work. The NSTL notifies the Engineering Manager, who contacts the gate owner directly.
...
**Residual limitation:** A gate owner who does not respond to the NSTL's direct contact, the automated notification, or the Engineering Manager's alternative-channel contact has exhausted the available mechanisms. At that point the gate remains unresolved and the Engineering Manager escalates to the oversight body under §0.5.
...
2. Copy that written statement simultaneously to the oversight body contact stored in the tracker (§0.2, oversight body contact procedure).
...
3. **Spike-event test:** In any month where the Gate 3 two-source check is scheduled (Month 2), the log must show at least 3 hours of coordination time for the week in which that check occurs. If the log shows less than 3 hours that week, it is flagged.
...
- **Month 2 monitoring point:** The Gate 3 two-source check (3–4 hours, occurring once) falls in Month 2. Combined with normal duties (~6.5 hours), Month 2 total may reach 9.5–10.5 hours. This will breach the 8-hour spike threshold. The Engineering Manager should anticipate this and decide in advance — at the Month 1 check-in — whether to preemptively redistribute other coordination duties in Month 2 or accept the temporary overrun as planned.

**Engineering budget impact — derived estimate:**

| Task | Frequency | Estimated time per instance | Monthly total |
|------|-----------|-----------------------------|---------------|
| Monday tracker update and gate status verification | Weekly