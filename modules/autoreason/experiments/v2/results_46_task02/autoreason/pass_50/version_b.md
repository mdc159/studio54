...
|---|---|---|
| Notifications lead / document owner | Priya Mehta | All technical decisions; 14-day and 60-day validation reviews (see §1.1.4 for full definition); escalation first contact; tier reassignment approval (§0.4) |
...
**Legal review deadline:** [Calendar date — to be inserted by project lead at kickoff, per §B milestone M3. This date is: SMS/email implementation start date (§B milestone M4) minus six weeks. The project lead reads the M4 date from §B and subtracts six weeks to produce a calendar date here. §B must be populated before this field can be set, which is why §B population is a distribution gate condition in §0.1. A formula is not a deadline. A calendar date is required.]

**Why this sequencing is possible:** §B defines M4 as the SMS/email implementation start date. §B is populated with calendar dates before this document is distributed. Therefore, the project lead can compute and insert the legal review deadline before distribution. If §B has not been populated, the project lead cannot complete this field, which means the distribution gate in §0.1 is not satisfied, which means the document is not distributed. The sequencing is self-consistent.
...
Option B requires written justification addressing: (i) the estimated size of the affected population (users without valid push tokens, measured per §0.3 step 4); (ii) why the safety risk to that population is acceptable without disclosure; and (iii) whether any regulatory requirement — including GDPR's transparency obligations — affects this decision. Justification for Option B is reviewed by legal counsel before the decision is recorded as final.
...
**Step 4 — Push token coverage measurement:** The fraction of users without a valid push token is measured during the first week of operation and reported at the 14-day validation review (§1.1.4). The result informs whether the Option B justification in step 2 remains valid given the actual affected population size, and whether the product lead's disclosure decision should be revisited.

The 10% threshold that appeared in an earlier version of this document has been removed. That threshold had no derivation — it was a number that created false confidence. Instead, the measurement result is reviewed at the 14-day review with the product lead and legal counsel present, and the threshold for triggering a disclosure decision revision is set at that review based on the actual data and the specific product and regulatory context. A threshold set from real data is more defensible than a threshold set in advance without analysis.
...
**Alternate approver when Priya Mehta is unavailable:** Technical sign-off authority passes to the highest-ranked available engineer per the tenure ranking in §A item 5 — the same criterion used for simultaneous unavailability in §0.2. The alternate approver must notify Priya Mehta of any reassignment made in her absence within 24 hours of her return.
...
If a reassignment elevates a security event to Tier 1, queued notifications created before the deployment timestamp will continue processing under the lower original tier. Operators must handle this explicitly:
...
- For elevations affecting security events: the deploying engineer checks queue depth for the affected event type before deploying. The decision criteria are:
  - If queue depth is zero or negligible (fewer than 100 notifications): deploy immediately. No drain required.
  - If queue depth is between 100 and 10,000 notifications: the deploying engineer decides whether to drain first, considering that a drain of this size typically takes under five minutes and does not block other event types. The decision and rationale are recorded in the deployment record.
  - If queue depth exceeds 10,000 notifications: drain before deploying. At the expected processing rate, 10,000 notifications represents approximately [X minutes — to be computed from §1.9 throughput figures before launch and inserted here]. If the drain duration is unacceptable given the urgency of the elevation, the on-call engineer escalates to Priya Mehta or the alternate approver, who decides whether to accept the race window or halt queue processing during deployment.

These thresholds are derived from the following reasoning: below 100 notifications, the practical user safety impact of processing under the wrong tier is negligible. Above 10,000, the population affected is large enough that the decision should not be discretionary. The middle range is where engineering judgment is appropriate. If this reasoning is wrong for a specific event type — for example, a security event where even one missed notification is unacceptable — the event type definition should note that explicitly, and the deploying engineer must drain regardless of queue depth.
...
The provisioning target uses 2× headroom over the estimated peak. This is a cost decision. Earlier versions of this document stated the 2× choice was justified by a cost comparison to 3× headroom, then left that cost figure as a placeholder. That is circular: the choice was documented as final before the information needed to evaluate it was available. This version corrects that.

**The cost comparison must be obtained before the provisioning choice is finalized.** The project lead obtains the cost difference between 2× and 3× headroom at projected load from the infrastructure vendor and records it in §A item 7 before any purchasing commitment. The provisioning choice is not final until §A item 7 is populated. If the cost difference is small relative to the project budget — a determination the project lead makes after seeing the actual figures — the project lead must bring the 3× option back to Priya Mehta for reconsideration before purchasing is finalized. "Small relative to project budget" means less than 15% of the total six-month infrastructure budget; the project lead applies this criterion to the actual figures and documents the result.

The case for 2× if the cost comparison supports it: (a) queue buffering absorbs spikes while auto-scaling responds, typically within 2–5 minutes, so short-window under-provisioning produces backlog rather than dropped notifications; (b) the validation process in §1.1.4 replaces assumptions with measurements within 14 days of launch, after which provisioning is adjusted based on real data; (c) baseline instances are set at 2× projected peak from day one, meaning effective starting headroom is higher than the steady-state target.

The honest case against 2×: if all three