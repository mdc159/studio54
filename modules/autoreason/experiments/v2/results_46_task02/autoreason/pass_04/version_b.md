...
| Synchronous preference check at routing | Couples routing latency to cache health; cache unavailability has explicit DB fallback; fail-closed means queue-for-later, not suppress |
| 4-instance poller fleet | 17× headroom over peak sustained load; full calculation in Section 3.2 |
| In-app written in same transaction | If push relay fails permanently, user has in-app record but no push — behavior on failure is specified in Section 2.3, not deferred to product |
| P0 rows isolated via separate outbox table | Eliminates contention between dedicated P0 poller and general fleet |
| Redis payload TTL is 24 hours | Covers realistic P2 backlog scenarios; worker falls back to PostgreSQL on miss with explicit behavior |

**What is not yet decided and requires sign-off before SMS goes live — SMS is excluded from month-2 launch until resolved:**

- Daily SMS spend caps and budget alert thresholds (Section 3.4). SMS ships in month 3, not month 2. Push, email, and in-app launch in month 2 without this dependency.
- Escalation path when outbox relay exhausts retries for auth OTPs — silent failure or operator alert? (Section 3.6). This must be resolved before month 2 because OTPs are P0 and launch with the system.
- Failure behavior for push-enabled, in-app-disabled users when push relay fails permanently — the system's current behavior is specified in Section 2.3; product must confirm this is acceptable, not design it.
...
### SMS Cost and Launch Sequencing

Twilio domestic US rates are ~$0.0079/message, but international SMS runs $0.05–$0.15/message. For a 10M MAU app with meaningful international distribution, blended cost is likely $0.02–$0.04/message. At 1M SMS/day, realistic daily cost is **$20,000–$40,000**. SMS without a spend cap is not a risk to acknowledge and defer — it is a risk that prevents launch of the SMS channel.

**SMS is excluded from the month-2 launch.** Push, email, and in-app ship in month 2. SMS ships in month 3 after finance and product have defined daily spend caps and budget alert thresholds. The architecture supports SMS from day one; the channel is toggled off at the dispatcher until caps are in place.
...
**The E3/E4 backup asymmetry is a real risk.** E4 owns reliability tooling, runbooks, and alerting — the components most likely to be needed during a production incident. E3's primary domain is preference management, the furthest from reliability tooling of any backup pairing. We address this directly: E3's month-1 co-build work with E4 is weighted toward the delivery pipeline failure handling and alerting infrastructure, not the preference system. E3 will not be expert in E4's domain by month 2, but E3 will have enough context to execute runbooks and escalate correctly. The shared month-2 on-call rotation means E4 is never the sole responder to an incident they authored.
...
[Priority Queues — 3 Redis Sorted Sets, AOF-persisted, Sentinel HA]
  ...
  notification_queue:p2  (likes, follows, digests)
   ...
   (PostgreSQL + S3)           (Redis sorted set + paged runbook)
          │
          ▼
   [Feedback Processor]
...
We use three separate Redis sorted sets with separate worker pools. The operational overhead is real and quantified here so the team can make an informed decision about whether it is worth carrying.
...
| Capacity scaling decisions | 3 | Worker pool sizes are set independently; resizing one does not automatically rebalance others |
| Starvation mitigation logic | 1 | P2 workers assist P1 above threshold — specified completely below |
...
P2 workers assist P1 when P1 is backlogged. The mechanism must answer four questions: what triggers migration, how many workers migrate, how they coordinate, and what triggers return.

*Trigger:* Each P2 worker checks P1 queue depth at the start of each poll cycle. This is a single `ZCARD notification_queue:p1` call, ~0.5ms. If depth exceeds `P1_ASSIST_THRESHOLD` (default: 10,000 items, configurable via environment variable), the worker enters assist mode.

*How many workers migrate:* P2 workers independently check the threshold and independently enter assist mode. We do not coordinate migration centrally. The consequence is that all 10 P2 workers may simultaneously enter assist mode, leaving P2 unserved. This is the correct behavior: if P1 has 10,000+ items backlogged, P2 should be fully deprioritized. P2 contains likes, follows, and digests — a multi-minute delay is acceptable. If the threshold is set too low and P2 starvation becomes a user-visible problem, raise the threshold.

*Return trigger:* A worker in assist mode returns to P2 after completing one P1 batch if P1 depth has fallen below `P1_ASSIST_THRESHOLD / 2` (5,000 items by default). The hysteresis prevents oscillation: workers do not flip between queues on every cycle when depth hovers near the threshold.
...
*Monitoring:* A metric `worker_assist_mode{pool="p2", target="p1"}` is emitted each cycle a P2 worker is in assist mode. An alert fires if more than 5 P2 workers are in assist mode for more than 10 minutes continuously — this indicates a P1 backlog that the worker pool cannot drain and requires human attention.

### 2.2 What the Outbox Pattern Does and Does Not Guarantee

The outbox pattern guarantees: a notification written to PostgreSQL will eventually be enqueued for delivery. It prevents the failure mode where the application crashes after writing to PostgreSQL but before writing to Redis.
...
Duplicates are handled at the worker layer via idempotency keys. The mechanism for handling concurrent duplicate processing is specified completely in Section 3.3 — including the race condition that a naive constraint-only approach does not handle.

### 2.3 In-App Notification Failure Mode — Behavior Specified, Product Confirmation Required

In-app notifications are written in the same PostgreSQL transaction as the outbox entry. If the outbox relay fails permanently (retries exhausted, row dead-lettered), the in-app record exists in the database.

**The failure mode:** A user with push enabled and in-app disabled has an in-app record in the database that is permanently invisible to them by their own preference. Their push notification failed. They receive nothing visible.

**What the system does in this state, specifically:**

1. The outbox row is dead-lettered with reason `relay_exhausted`.
2. The dead-letter record includes the user_id, notification type, and channel.
3. For P0 notifications (OTPs, security alerts), the dead-letter event triggers an operator alert and an escalation to a secondary channel if configured — specified in Section 3.6.
4. For P1/P2 notifications, no escalation occurs. The notification is lost from the