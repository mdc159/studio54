...
2. **Notification rate per DAU:** assumed per event type in the table below. These figures are drawn from general social app benchmarks with no product-specific validation — see §1.1.2 for the explicit limitation this creates.
...
The per-event rates below are drawn from general social app benchmarks, not product-specific data. This is a material limitation. Notification rates vary substantially across social product types: a platform with algorithmically amplified content (where a single viral post generates likes at high volume across a large follower graph) will produce radically higher per-user like notification rates than a close-friends network where content distribution is narrow. A real-time messaging product will produce higher DM notification rates than a passive content feed.
...
- **Spike buffering and the 2–5 minute auto-scaling gap:** Auto-scaling under cloud providers typically provisions new instances within 2–5 minutes. During that window, if an unprovisioned spike occurs, new capacity is not yet available. The system handles this gap through queue buffering: all notification jobs are enqueued before processing, so a spike above the current processing capacity accumulates in the queue rather than dropping requests or returning errors. The relevant question is whether the queue can absorb 2–5 minutes of spike volume before the spike either subsides or new capacity comes online. At 130/second provisioning target and a hypothetical 3× spike (390/second), the queue accumulates approximately 390 × 150 seconds = 58,500 jobs during a 2.5-minute gap. At a message size of approximately 1KB per notification job, this is ~58MB of queue backlog — well within the capacity of standard queue infrastructure. Queue depth alert thresholds are set in §1.1.4; a sustained spike that does not subside within 5 minutes triggers escalation before the queue reaches problematic depth.
- Provisioning target with 2× headroom for compounded assumption uncertainty: **~130 notifications/second**

All queue consumers and delivery workers are deployed with auto-scaling configured from day one, with baseline instance counts set to handle 2× the projected peak from launch. This means the system is intentionally over-provisioned at launch. The cost of that over-provisioning is explicitly accepted as insurance against the compounded assumption uncertainty.
...
**Pre-distribution check:** Before this document is distributed to any stakeholder, the project lead must verify that the named owner above reflects current team composition. This verification is a named step in the project kickoff checklist, not a reminder — if the checklist is not completed, distribution is blocked. The project lead signs off on the checklist; their signature is the evidence that the named owner was verified at distribution time.
...
The 14-day review is mandatory. It produces a written update to this section that becomes the authoritative figure for all subsequent planning. The review is not deferred because nothing appears wrong.

**Enforcement mechanism:** The review date is entered as a calendar event with the team lead and product lead as required attendees at project kickoff. Missing the review requires explicit sign-off from the team lead, who must document why it was deferred and set a new date within 72 hours. The consequence of missing the review without sign-off is that the team lead escalates to the product lead; the review does not simply slip. The notifications lead owns scheduling the review; the team lead owns enforcing that it happens.
...
All decision rules target the same headroom standard: **baseline instance counts set to handle 1.5× the measured sustained peak**. This standard applies in all cases.
...
- If actual peak is 50–100% above estimate: increase baseline instance counts until the new baseline handles 1.5× the measured peak.
...
**Tier 1 — Security and account events.** Examples: password reset, email address change, login from new device, account recovery. Delivery is concurrent across all available channels (push, email, SMS, in-app). No batching. No quiet-hour suppression. Failure on any single channel does not suppress delivery on others. Delivery is attempted and logged on every channel regardless of user preferences, because the user's ability to act on a security event must not depend on which channels they have opted into. See §1.3 for the full Tier 1 delivery path.

**Tier 2 — Direct social interactions.** Examples: direct messages, mentions, comments on the user's own content. Delivered promptly (target: within 60 seconds of the triggering event for an online device). Subject to quiet-hour suppression. Not batched. Every Tier 2 notification is written to the in-app store before push is attempted — see §1.2.2 for the implementation guarantee.
...
The tier system exists to prevent two failure modes. First, it prevents a backlog of Tier 3 ambient notifications from delaying Tier 1 security events — they run on separate queues with separate workers. Second, it prevents the per-user notification volume from being dominated by likes and follows, which are high-volume, low-urgency events that would obscure higher-value interactions if delivered immediately and individually.

---

### 1.3 Tier 1 Delivery Path

Tier 1 notifications (security and account events) require a delivery approach distinct from Tier 2 and Tier 3 because the cost of non-delivery is high and the user cannot be expected to have any particular channel preference configured. A user who has never set up push notifications still needs to receive a password reset email.

**Delivery behavior:** All available channels are attempted concurrently at the moment the Tier 1 event is created. "Available" means the channel infrastructure is operational, not that the user has opted into it — user channel preferences do not suppress Tier 1 delivery. The rationale: a user who disabled marketing emails did not consent to missing account security alerts. If this interpretation conflicts with applicable regulations in a specific jurisdiction (some privacy frameworks require opt-in for all commercial communications regardless of category), legal review is required before launch in that jurisdiction. This is flagged as an open item requiring legal sign-off.
...
- **Email:** Sent immediately via the transactional email provider (see §1.2.3). Not batched. Uses a dedicated sending domain separate from marketing email to protect deliverability — a security email arriving in spam because the sending domain was flagged for marketing volume is a Tier 1 delivery failure.
- **SMS:** Sent immediately via SMS provider (see §1.2.4). Tier 1 is the only tier that uses SMS proactively — for Tier 2 and Tier 3, SMS is only used as a fallback when the user has no other reachable channel.
- **In-app:** Written to the in-app store immediately, with a distinct visual treatment (red badge, persistent until acknowledged) that distinguishes it from social notifications.
...
**Delivery confirmation logging:** Each channel attempt is logged with timestamp, channel, outcome (delivered/failed/retried), and the error code if failed. This log is the source of truth for incident investigation and is retained for 90 days. The system does not treat push delivery acknowledgment from APNs/FCM as proof the user saw the notification — it treats in-app acknowledgment (the user tapping the notification) as the only confirmed-read signal.

---

### 1.2.3 Email Notifications

Email is used for Tier 1 security events (always), Tier 3 digests (when the user has opted into email digests or has no other reachable channel), and re-engagement sequences (out of scope for this design; flagged for the product team).

**Provider choice:** A managed transactional email provider (SendGrid or AWS SES) rather than self-hosted SMTP. The operational cost of maintaining deliverability for a self-hosted mail server — IP warming, DMARC/DKIM/SPF configuration, bounce handling, feedback loop management with major ISPs — is not justifiable for a four-person team. The tradeoff is vendor dependency and per-message cost. At 1.3M notifications/day with a conservative estimate that 30% result in an email (390,000 emails/day), SendGrid's cost at their standard tier is approximately $0.001/email, or ~$390/day. This is the accepted cost of not operating mail infrastructure.

**Dedicated sending domains:** Security emails (Tier 1) use a sending domain separate from digest/engagement emails. This is not optional — a shared sending domain means a spam complaint rate spike on digest emails can cause security emails to land in spam, which is a Tier 1 delivery failure with user-facing consequences.

**Bounce and complaint handling:** Hard bounces (invalid address) are removed from the email registry immediately. Soft bounces (temporary delivery failure) are retried up to 3 times with 15-minute spacing before treating the address as temporarily unreachable. Complaint callbacks from ISPs (user marked as spam) suppress all non-Tier-1 email to that address. Tier 1 email continues to that address because suppressing security emails based on a spam complaint would be a worse outcome than the annoyance of receiving an unwanted security alert.
