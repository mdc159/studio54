...
10M MAU implies roughly 2–3M DAU on a social app (industry baseline: 20–30% DAU/MAU ratio; working figure: 2.5M DAU). **This ratio must be validated against actual product analytics within 30 days of launch. Validation ownership and process are described in §1.8.**
...
Digest notifications are pre-computed by a scheduled job that runs at 06:00 UTC daily. The job aggregates each user's unread activity using a snapshot of read/unread state. The snapshot is taken once at job start and is fixed for the entire run. All users in that run are evaluated against the same snapshot timestamp.
...
There is a known staleness consequence of this design: the job processes users sequentially over multiple hours, and a user processed near the end of the run is evaluated against a snapshot that may be 4–6 hours old relative to their processing time. A notification received at 06:01 UTC will not appear in the digest for any user, regardless of when during the run their digest is computed. This boundary is accepted: the alternative — taking a fresh snapshot per user or running live reads throughout the job — produces inconsistent results across users and is operationally harder to reason about. The snapshot approach gives every user in a given run a consistent, if not current, view of their activity.

A user who receives a late-night notification that is not captured in the morning digest will see it in the following day's digest. This is a known product behavior, not a system error, and should be reflected in user-facing documentation of how digests work.
...
A 5-minute TTL means a user who changes their preferences may receive notifications on the old preference for up to 5 minutes. This is acceptable for routine preference changes. For cases where staleness is not acceptable — account suspension, deletion — the system uses direct cache writes rather than event-driven invalidation, as described below.

**Cache invalidation: what it solves and what it doesn't:**

The proposal uses two mechanisms for reducing preference cache staleness below the 5-minute TTL:

1. **Direct cache write on critical state changes:** When an account is suspended or deleted, the handling service writes the updated state directly to Redis immediately, synchronously, before returning. This closes the staleness window for those specific events: the cache entry reflects the new state as soon as the operation completes, not after a consumer processes an event. The tradeoff is that the writing service must have a direct Redis dependency, which is acceptable for the account management service but not appropriate as a general pattern.

2. **Cache invalidation events via `account-events` queue:** For preference changes that are not suspension or deletion, the preference service publishes an invalidation event to the `account-events` FIFO queue. Workers consume this event and evict the affected cache key. This reduces staleness for users who change preferences and then immediately trigger a notification — a common pattern when, for example, a user turns off notifications for a specific subtype because they are receiving too many. However, this mechanism is asynchronous: between the time the event is published and the time each worker processes it, those workers continue serving the stale cached preference. During consumer lag, this window could exceed the 5-minute TTL it was intended to improve. This mechanism reduces typical staleness; it does not provide a consistency guarantee.
...
1. The account deletion event causes the account management service to write a deletion record to a `deleted_users` table in PostgreSQL and simultaneously write the deleted status directly to Redis (as described above — synchronous, not event-driven).
...
3. All delivery workers check deleted status before processing any notification. The check queries the Redis cache for deleted status. Because deletion writes directly to Redis synchronously (step 1), the cache reflects deletion as soon as the operation completes. Workers that have the old preference cached will find the deletion flag on their next Redis read for that user.
...
**Bloom filter for deleted user check:**

Workers use a Redis bloom filter as a fast-path check for deleted users. A bloom filter has zero false negatives by construction: if the filter reports that a user is not in the deleted set, that result is guaranteed correct and requires no further verification. The worker proceeds without a database lookup.
