...
**Last revised:** [date]
**Status:** Draft — SMS/email implementation gated on legal review defined in §0.3
...
**Simultaneous unavailability:** If both Priya Mehta and the project lead are simultaneously unavailable, decision authority passes by a single unambiguous criterion: years of tenure on this team, with ties broken by date of hire (earlier hire ranks higher). This ranking is recorded explicitly in §A item 7 and must be populated at kickoff — it is not determined in the moment of an incident. The highest-ranked available engineer assumes temporary ownership of all time-sensitive decisions, contacts the product lead within 24 hours using the kickoff checklist contact information, and designates a permanent replacement within 48 hours. The ranking is reviewed and updated whenever team composition changes.
...
**Review required:** Tier reassignments require sign-off from Priya Mehta (technical correctness and load impact), the product lead (user experience and product intent), and legal counsel if the change affects a channel with regulatory implications.

**Handling in-flight notifications during transition:** Tier reassignments are implemented as two-step deployments. Step one: update the event type definition in the configuration store. Step two: the change takes effect for all notifications created after the deployment timestamp. Notifications already in queue are processed under their original tier assignment. The deployment timestamp is logged and retained for 90 days to support investigation of any delivery anomalies around the transition.
...
**The 2× headroom choice is a cost decision, not an engineering determination.** The argument for 2× over higher multiples is not that 2× is better-grounded in data — it has exactly the same epistemic status as a 6× estimate. The argument is: (a) 2× is cheaper; (b) queue buffering absorbs spikes while auto-scaling responds, limiting the consequence of short-window under-provisioning; (c) the validation process in §1.1.4 replaces assumptions with measurements within 14 days of launch, limiting exposure to unvalidated estimates; and (d) the cost of revising upward after measurement is explicitly accepted as part of the design.
...
**Benchmark sourcing — important caveat:** The per-event rates below are drawn from published industry analyses of social apps of comparable scale, not from this product's own data. The specific sources are recorded in §A item 8 and must be populated before infrastructure purchasing decisions are made. These are placeholders because the appropriate benchmarks depend on the product's category — a messaging-first app has a materially different notification profile than a photo-sharing or interest-graph app — and that categorization must be confirmed by the product lead before the rates below are treated as applicable.
...
The full impact of the high-end assumption: at 5 recipients per sender, DM notifications rise from 500,000 to 2,500,000 per day. Total daily volume rises from approximately 1.3M to approximately 3.3M — a 2.5× increase in total volume. The downstream provisioning target of approximately 130 notifications/second would need to be approximately 325/second. This is a material difference in infrastructure cost and must be resolved before any purchasing commitment.
...
**The validation schedule and infrastructure purchasing timing:** Infrastructure purchasing decisions may need to be made before the 14-day review produces measured data. The validation schedule is therefore not a mechanism for informing initial purchasing decisions — its purpose is to replace assumed values with measured ones as quickly as possible after launch, and to trigger escalation if measurements reveal that initial decisions were substantially wrong.

**Review schedule:**

*14-day review (mandatory):* Produces a written update replacing assumed per-user notification rate, peak multiplier, and Tier 3 "immediate" preference fraction with measured values. This update becomes the working figure for subsequent planning. Day-14 data may not represent