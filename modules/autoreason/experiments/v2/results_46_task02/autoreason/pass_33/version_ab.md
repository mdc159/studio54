...
This revision synthesizes the strongest elements of two prior versions. The table below records each problem, the resolution adopted, and its location. Every section referenced in this table exists in this document.
...
| 12 | Return of duties after NSTL absence conditioned on EM acknowledgment; EM non-performance creates ambiguous dual-holding state | EM acknowledgment replaced with documented handoff entry signed by two separate repository commits in specified order; handoff valid without EM acknowledgment; Deputy EM notified by repository commit to designated branch, not by email alone | §0.2 |
...
| 23 | NSTL Backup read access before activation not stated; surfacing mechanism depends on access not established | NSTL Backup has read access to tracker and time log from project kickoff; write access granted only at activation; read access is sufficient for the surfacing function | §0.2 |
| 24 | Deputy EM notification at handoff return has no defined obligation; email to unresponsive recipient is not a reliable independent record | Notification made by repository commit to a designated branch; commit constitutes the independent record regardless of Deputy EM responsiveness; Deputy EM has defined 24-hour acknowledgment obligation with escalation path if unmet | §0.2 |
...
**NSTL Backup — access, activation trigger, contest window, handoff, and scope:**

**Read access before activation:** The NSTL Backup has read access to the tracker and the NSTL's time log from project kickoff, granted at the same time the N