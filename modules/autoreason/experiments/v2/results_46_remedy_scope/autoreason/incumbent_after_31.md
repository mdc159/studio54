...
For each decision, the authority to select the fallback is held by the product owner named in Section 7.1. Engineering does not have unilateral authority to make product behavior decisions. What engineering does have is authority to block deployment if a decision is not made by the deadline in Section 7.1 — and the fallback is what gets deployed if the product owner makes no selection by that deadline, because the product owner's silence is itself a choice that the product owner owns.
...
**1. Four isolated priority queues (P0–P3)** with TTL enforcement and a token bucket for starvation prevention. The token bucket guarantees minimum throughput for lower-priority queues under sustained high-priority load. The token bucket parameters — refill rates, bucket sizes, and the conditions under which P2/P3 can still be deferred despite the token bucket — are fully specified in Section 3.2, not referenced to it.

**2. ID-based queue entries with a Redis Sorted Set for processing state.** Workers fetch full payloads from PostgreSQL on dequeue. Crash recovery reclaims orphaned entries within a 90-second window. The derivation of that bound is in Section 4.2. The derivation is present in Section 4.2, not merely cited. Worst-case delivery outcome is duplication, not loss.
...
**Limitation of this revision:** 14 active hours is itself an assumption from general social-app patterns, not a measured value for this application. It is not validated; it replaces one assumption with another. If actual active hours are 16–18, the computed peak rates are 14–22% lower than stated. The direction of the correction (toward lower peaks) is acceptable for capacity planning — it means the system is sized conservatively — but the magnitude of the correction is unknown until month-1 data is available.

**Calibration requirement:** The on-call rotation owner recalibrates active hours and multipliers at the month-1 checkpoint using actual traffic data. The procedure is in Section 7.1. Until recalibration, all peak rate figures carry ±25% uncertainty.
...
The extreme row's peak rate (~9,643/sec) is approximately 4.9× the plan row's (~1,964/sec) while daily volume is only 3.6× higher. This is expected: both daily volume and the peak multiplier are higher in the extreme row, and the multiplier compounds against the higher base. It is not an error in the methodology.
...
**Planning decision:** Size for 45M/day (~1,964/sec sustained peak). The traffic response matrix in Section 1.3 covers the full range from plan through 162M/day, with explicit infrastructure states for each threshold. Coverage is demonstrated there, not asserted here.
...
**SMS cost exposure from uncapped volume:** At 2% SMS share and $0.01/message, 1.5M additional notifications/day adds $15,000/day in unbudgeted SMS cost. At 5% SMS share, that exposure reaches $75,000/day. The full cost table in Section 1.2 provides the basis for these figures.

**[PRODUCT DECISION REQUIRED — Daily Spam Threshold]:** Define what daily notification volume is excessive and what action to take (suppress, hold for review, notify sender). **Fallback if no decision before launch: hard cap of 200 notifications/user/day with automatic suppression above that threshold.** This fallback is deployed if the product owner named in Section 7.1 has not made a selection by the deadline specified there. The product owner owns the fallback outcome.

**Recipient fanout:** Fanout is capped at 10,000 recipients per event. Excess recipients are processed in batched jobs at P2 priority. The worst-case fanout completion time under sustained P0/P1 congestion is derived from the token bucket parameters in Section 3.2. The derivation is:

```
P2 minimum guaranteed rate (from Section 3.2): 200 tokens/sec
...
Rounding up for queue overhead and batch processing latency: ~10 minutes worst case.
```

**Correction from prior version:** The 45-minute figure in earlier drafts was an error — it assumed P2 received only 1/15th of its guaranteed rate under congestion, which contradicts the token bucket guarantee. The correct worst-case under the token bucket parameters in Section 3.2 is approximately 10 minutes. If P2's guaranteed minimum is lowered below 200/sec, this bound increases proportionally.
...
**Accounting note — in-app:** In-app volume is counted separately because it generates distinct database writes (unread badge count, notification feed row). It does not consume push quota but does consume write throughput. The 