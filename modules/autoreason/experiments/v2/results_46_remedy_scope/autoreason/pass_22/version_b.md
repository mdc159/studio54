...
**Revision note:** This version addresses ten structural problems identified in review. The most consequential changes are: (1) worker topology reduced from 16 to 6 deployments with explicit justification; (2) FCM rate limit uncertainty elevated to a hard architectural dependency with a resolution path before design commitments are made; (3) missed-checkpoint procedure replaced with traffic-conditional logic; (4) peak multiplier variance added to the sensitivity table; (5) burst allowance corrected so it cannot exceed the research ceiling; (6) Redis failover behavior during traffic events specified in detail; (7) deduplication interaction documented with a decision tree; (8) fanout cap reframed as a consistency decision, not a latency decision; (9) Section 1.4 written in full; (10) staffing analysis added as a first-class design input. Each change is traceable to the specific criticism it addresses.
...
**The staffing constraint is a design input, not a label.** Four engineers over 6 months cannot build and safely operate a system of arbitrary complexity. Section 1.5 contains an explicit staffing analysis that drove several architectural decisions, most visibly the reduction of worker deployments from 16 to 6. Every place in this document where a simpler approach was chosen over a more capable one, the staffing constraint is the reason.
...
**3. Six worker deployments covering four channels and two priority tiers.** Not sixteen. The reduction from the theoretical maximum of sixteen (4 channels × 4 priorities) to six is a deliberate staffing-driven decision. The cost is some loss of priority isolation within non-push channels. The benefit is an operational surface area that 4 engineers can actually manage. The tradeoff is explained in Section 1.5 and the acceptable-loss analysis is in Section 3.3.

**4. Redis primary/replica with bounded failover.** Section 6.2 specifies exactly what happens to in-flight queue entries during a promotion window, whether the processing sorted set is preserved, and how workers behave during the unavailability gap. "Recoverable" is demonstrated, not asserted.
...
**Peak multiplier variance:** The sensitivity table uses a variable peak multiplier, not a fixed 3×. Higher-engagement scenarios imply more real-time interaction and stronger evening concentration, which increases the ratio of instantaneous peak to daily average. Using a fixed 3× while noting correlation risk is inconsistent; the table reflects that inconsistency's resolution.
...
| 30M–55M/day | On plan or modestly above. Scale push-worker to 12 replicas, inapp-worker to 6 replicas. These counts are justified in Section 1.5 as the plan-rate baseline plus 50% headroom. Document and notify engineering lead. |
...
Scaling actions in the 30M–55M/day band are reversible. Step-down requires: traffic review completed and documented, engineering lead sign-off, and 7 days of post-review data confirming volume below 45M/day.
...
**Named decision owner:** Engineering lead and named backup are recorded in the runbook and deployment checklist before launch. Placeholder text fails the pre-flight check.
...
# Worker pod counts by deployment (6 deployments, not 16 — see Section 1.5)
...
| 45M–80M/day | 1,575–2,800/sec | Moderately above plan | Push and in-app co-saturate — check both queues first. Scale push-high: `kubectl scale deployment push-worker-high --replicas=16