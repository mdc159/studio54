...
The critic identified thirteen problems with Revision 2. This revision addresses each one directly. The table below maps each finding to its resolution and the section where the resolution can be verified within this document. Sections that were absent in Revision 2 are now present. No finding is marked resolved unless the resolution is auditable in the current document.

| Finding | Problem | Resolution | Verifiable at |
|---|---|---|---|
| 1 | Revision table unverifiable — most sections absent | All sections in the ToC are now present in this document | Every section below |
| 2 | CI completeness guarantee visibly failing | CI mechanism description removed; completeness is now a reader-verifiable property of this document | This table |
| 3 | Section 7.1 absent; pre-flight script unauditable | Section 7.1 is present; pre-flight logic is specified inline, not by reference to an absent script | 7.1 |
| 4 | Five unresolved decisions with no fallback behavior | Each unresolved decision now has an explicit fallback behavior if no decision is made before launch | 1.1, 2.2 |
| 5 | FCM/APNs two-branch Redis sizing not present | Both branches are now specified in full | 1.4, 6.1, 6.2 |
| 6 | 90-second crash recovery bound asserted, not shown | Derivation is present in Section 4.2 | 4.2 |
| 7 | 45-minute fanout bound asserted, not derived | Derivation is present in Section 3.2 | 3.2 |
| 8 | Month-1 checkpoint truncated mid-sentence | Paragraph completed in full | 1.1 |
| 9 | Section 1.3c listed but absent | Section 1.3c is present | 1.3c |
| 10 | Section 1.5 staffing arithmetic claimed but absent | Section 1.5 is present with full arithmetic | 1.5 |
| 11 | Extreme row (162M/day) vs. runbook threshold (>80M/day) unexplained | Traffic response matrix now covers the full range; extreme row is explicitly addressed | 1.3 |
| 12 | Redis two branches not comparable because both absent | Both branches are present and directly comparable | 6.1, 6.2 |
| 13 | Scale-down fallback and CI check share the same external repo — single point of failure | External repo dependency removed for both; scale-down procedure is self-contained in this section; CI mechanism replaced with reader-verifiable completeness | 1.3, this table |
...
- Traffic spike response → **Section 1.3** (CLI commands embedded; all deployment names are in the procedure)
...
- Scale-down needed → **Section 1.3** (scale-down procedure is self-contained in this document; it does not depend on any external repository)
...
The Redis provisioning decision (Section 6.1) depends on two prior decisions in order: (1) the cross-channel deduplication retention window (Section 2.2), then (2) the FCM rate verification outcome (Section 1.4). Both must be resolved before a Redis provisioning option is committed.

**Unresolved product decisions:**

Five decisions require named humans before the system can launch. Each has a specified fallback behavior if the decision is not made. The fallbacks are conservative defaults that keep the system operational; they are not optimal product choices. The decisions and their fallbacks are:

1. Burst allowance (Section 1.1) — fallback: 3 notifications above ceiling per 5-minute window
2. Daily spam threshold (Section 1.1) — fallback: no daily cap enforced; spam risk accepted until product decides
...
**Named owners and deadlines are recorded in Section 7.1.** The pre-flight checklist in Section 7.1 must be completed before any production deployment. It does not depend on an external script or repository.
...
Multipliers increase with engagement level because more real-time interaction compresses activity into shorter windows. The extreme row's peak rate is 4.76× the plan row's while daily volume is 3.6× higher. This superlinear scaling is a consequence of the methodology, not an error.
...
A cohort of highly active users generating 50+ notifications/day stresses three subsystems: per-user queue partitioning, Redis key distribution, and the daily spam control (if any). These are two distinct problems requiring two distinct decisions.
...
**Problem 2 — Daily spam threshold:** A user generating 50 notifications/day is not constrained by the 20/hour ceiling (50/day is well below the 480/day extrapolation). There is currently no daily cap. Infrastructure mechanisms prevent one power user from monopolizing queue capacity, but they do not prevent objectively excessive daily volume.

**[PRODUCT DECISION REQUIRED — Daily Spam Threshold]:** Define what daily notification volume is excessive and what action to take (suppress, hold for review, notify sender). This is separate from the burst allowance decision. **Fallback if no decision before launch: no daily cap enforced; spam risk accepted until product decides.** Owner and deadline: Section 7.1.
...
**SMS volume sensitivity:** The 2% figure is an assumption. SMS is the most cost-sensitive channel (~$0.01/message at standard rates). At 2%, SMS costs approximately $9,000/day at list price — negotiate volume pricing before launch. If SMS grows to 5% of volume, daily cost exceeds $22,000. The SMS opt-out compliance owner (Section 5.4) is responsible for monitoring this.
