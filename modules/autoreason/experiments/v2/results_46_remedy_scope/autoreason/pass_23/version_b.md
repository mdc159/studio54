...
### Revision 2 — Addressing Structural Deficiencies

---

## Revision Notes

This revision addresses twelve specific deficiencies identified in the previous version. Each is resolved directly:

1. **Truncated matrix** — Section 1.3 is complete. The Redis Option B failover procedure is no longer cut off.
2. **FCM rate assumption** — P1 delay figures are removed from the Executive Summary. They appear only in Section 1.4, clearly marked as unverified estimates, with a verification procedure that has a named owner and a hard deadline.
3. **Missing staffing analysis** — Section 1.5 is now present and contains the full justification for the 6-deployment decision.
4. **Missing deployment checklist** — Section 7.1 is now present with all required fields.
5. **Placeholder in product decision gate** — The fanout cap gate now contains explicit options rather than a template string.
6. **Burst allowance justification** — The invented "research" citation is removed. The burst allowance is presented as a product decision with honest uncertainty bounds.
7. **Month-1 accountability gap** — The checkpoint now has an independent escalation trigger that does not rely on the person who may have missed the review.
8. **Sensitivity table contradiction** — The plan-row 3× multiplier is now justified, not simply asserted.
9. **Scale-down with unavailable lead** — Section 7.2 is now present and covers this case.
10. **Auto-scaling gap** — Month-1 after-hours spike risk is now explicitly addressed with a defined response procedure.
11. **Missing deduplication section** — Section 2.2 is now present with full arithmetic.
12. **Missing Redis failover section** — Section 6.2 is now present with step-by-step promotion window behavior.
...
- **Per-user rate limiting:** Concentrates write load on specific Redis keys. The per-user rate limit is 20 notifications/hour. The burst allowance is a product decision. **[PRODUCT DECISION REQUIRED]:** Engineering recommends a burst allowance between 3 and 5 (allowing 23–25 notifications in a short window above the sustained hourly rate). The honest basis for this range is operational judgment about spam perception, not a citable study. Product must select a value in this range before launch. The engineering team will implement whatever value product selects. This is a deployment checklist gate recorded in Section 7.1.
...
- **Recipient fanout:** Fanout is capped at 10,000 recipients per event. Excess recipients are processed in batched jobs over 5 minutes. **This is a product-visible consistency decision, not merely a latency decision.** During the 5-minute window, some followers have received the notification and may have already acted on the underlying content while others have not. The engineering rationale for the cap is preventing a single viral event from saturating queue capacity for all other users. **[PRODUCT DECISION REQUIRED]:** Product must select one of the following before launch:
  ...
  - **Option C — Reject the cap and set a higher limit:** Product sets the cap. Engineering will re-evaluate queue saturation risk at the chosen value and may require infrastructure changes.
...
| **Plan** | **30%** | **3M** | **15** | **45M** | **3× (see rationale above)** | **~1,575/sec** |
...
| 55M–80M/day | Moderately above plan. Execute the 45M–80M/day row of the traffic response matrix in Section 1.3 in full. Escalate to engineering lead immediately. |
...
**Redis provisioning dependency:** The 45M–80M/day response branch has two paths depending on which Redis option was selected in Section 1.3a. Operators follow the correct branch by checking the recorded option in Section 7.1 — they do not re-evaluate the tradeoff under pressure.
...
|---|---|---|---|---|---|
| < 7.5M/day | < 260/sec | Well below plan | Verify deployment names. Scale down: `kubectl scale deployment push-worker-high --replicas=2`