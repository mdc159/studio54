...
**The power-user pattern (low DAU/MAU × high per-user notification rate):** A social app with 15% DAU/MAU but a small cohort of highly active users generating 50+ notifications/day is a plausible and common pattern. Total daily volume in this scenario is below the plan figure, which means the volume-based sensitivity table does not flag it as a risk. However, it stresses three specific subsystems in ways total volume does not capture:
...
- **Deduplication set sizes:** Deduplication is keyed per user. A power user generating many notifications in a short window produces a larger deduplication set for that user. The per-user deduplication window is 60 seconds; entries older than 60 seconds are expired. Memory impact is bounded by the 60-second window, not by per-user notification rate.
...
The 3× peak multiplier assumes a two-hour evening window. **Intraday distribution matters independently of daily totals** — a day with 45M total notifications but a 5× peak in one hour would appear "on plan" in the daily volume column while actually requiring a higher-tier response. Section 1.3 addresses this with a parallel per-second threshold column in the traffic response matrix. This assumption must be revisited with real traffic data at the month-1 review.
...
**Runbook dependency:** The runbook is a hard prerequisite for this procedure, not a month-1 deliverable. The runbook must exist before production launch. The deployment checklist gate (Section 7.1) blocks launch if the runbook location field is empty.
...
- *Manual, no runbook:* This state cannot occur in production. The runbook is a launch prerequisite enforced by the deployment checklist gate (Section 7.1).

### 1.3a Redis Provisioning Decision — Cost Tradeoff Resolution

The traffic response matrix for the 45M–80M/day band depends on whether Redis is pre-provisioned at 80M/day capacity. This cannot remain unresolved until a traffic event occurs. The decision