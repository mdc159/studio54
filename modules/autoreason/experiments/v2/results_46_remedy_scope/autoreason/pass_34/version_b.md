...
**Decision 2 — SMS cost co-sign requirement:** The daily spam threshold directly controls SMS cost exposure quantified in Section 1.2. Decision 2 therefore requires two signatures: the product owner and the person with budget authority over SMS costs. Section 7.1 names both. If they are the same person, one signature suffices. If they disagree, the more restrictive threshold applies until the disagreement is resolved in writing.

**If the SMS budget owner role is not filled before the deadline:** Engineering cannot independently determine whether the role is filled — that is an organizational fact, not a technical one. The pre-flight checklist in Section 7.1 includes an explicit gate: "SMS budget owner identified and has reviewed Section 1.2 cost table." If that gate is not cleared, the checklist fails. The product owner is responsible for clearing organizational blockers that appear on the checklist; engineering surfaces them and deploys the conservative fallback if they are not cleared. This is the enforcement mechanism: the gate fails visibly, the product owner owns the gap, and the 200/day cap applies.
...
**Option A is the fallback.** The consistency gap is visible in logs and delivery receipts but is not user-visible in most product surfaces (users see the notification when it arrives, not its timestamp relative to other recipients). Option B's head-of-line blocking risk is a worse tradeoff for a system where P0 notifications (Section 3.1) include time-sensitive content. If the product surface makes delivery timestamp consistency visible to users — for example, a "seen by" list that shows ordered delivery — this decision should be revisited before launch.
...
**Engineering's prerequisite:** Before this is framed as a product decision, engineering must answer: does the notification feed deduplicate at read time by event ID? This answer must be recorded in Section 7.1 before the pre-flight checklist closes.

**Sequencing problem:** This document is being circulated before that engineering answer exists, which means Decision 4 is framed as a product decision prematurely. To correct this: Decision 4 is on hold pending the engineering answer. If the feed deduplicates at read time, the infrastructure window is largely irrelevant and Decision 4 can be resolved by engineering alone with no product owner input required. If the feed is append-only, Decision 4 escalates to the product owner with the duplicate-notification risk made explicit. The pre-flight checklist gate for Decision 4 reads: "Engineering has recorded feed deduplication architecture in Section 7.1 and has either resolved Decision 4 independently or escalated to product owner with written risk statement."

**Decision 6 — Runbook gate and structural constraint:** The on-call rotation owner can waive the runbook gate — but the person who bears operational risk is also the person who can waive the protection designed to manage that risk. This is a structural incentive problem: the person most likely to say "we'll figure it out" is the one who gets to make that call unilaterally.

The constraint applied here: a written waiver is not open-ended. It must include a post-launch runbook deadline — no more than two weeks after launch — and names an escalation owner if the deadline is missed. If the on-call rotation owner will not commit to a specific post-launch deadline in writing, the waiver is not valid and the gate remains failed. This does not prevent launch — it prevents the waiver from creating a permanent exception with no closure condition.
...
**What this means for the intermediate rows:** The high-retention/conservative row uses 2.2× (same as plan) because conservative notification policy suppresses campaign-driven peaks even with high DAU. The moderate-retention/aggressive re-engagement row uses 2.5× because the aggressive campaign behavior is the source of the higher multiplier, not the DAU level. These multiplier assignments are judgments, not measurements. If month-1 data shows the actual peak multiplier is materially different, the sensitivity table should be recalibrated before the month-3 infrastructure review in Section 8.

**Compounding uncertainty:** The sensitivity table has three compounding assumptions per row: DAU/MAU ratio, notifications-per-user, and peak multiplier. The stated ±25% uncertainty on peak rates reflects uncertainty in the peak multiplier alone. Joint uncertainty across all three inputs is larger. The extreme row's ~9,643/sec could plausibly be 7,200–12,000/sec under joint