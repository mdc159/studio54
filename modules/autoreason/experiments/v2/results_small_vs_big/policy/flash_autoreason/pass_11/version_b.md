...
1.  Inputting any customer PII, financial data, or database schemas into any AI tool (Base Fact: Handles customer PII and financial data, GDPR obligations, Incident #1).
...
*   **Introduction:** Added "(Base Fact: Legal review)" to explicitly link the legal risks to the legal review.
*   **2. Permitted Uses:** Added "If all 80 licenses are in use, employees must relinquish their license when not in use, or wait until one becomes available". This clarifies what happens when all licenses are used, addressing the license availability issue.
*   **3. Prohibited Uses:**
    *   Prohibition 5: Changed "Using any AI tool other than GitHub Copilot Business for code generation" to "Using any AI tool other than GitHub Copilot Business for code generation without express written permission from the Director of Engineering." This allows for flexibility if, in the future, another tool is approved, while still maintaining control.
    *   Prohibition 6: Changed "pre-commit review" to "review." Removed "verification of licensing terms and potential copyright issues" and replaced it with "confirm that the code does not include verbatim content from another source, and that the license is compatible with our existing codebase." This provides more specific, actionable criteria for the code review and makes it more enforceable.
*   **4. Enforcement:**
    *   Enforcement 1: Changed "verification of licensing terms and a check for potential copyright issues" to "confirm that the code does not include verbatim content from another source, and that the license is compatible with our existing codebase". This mirrors the changes in Prohibition 6 for consistency and enforceability.
    *   Enforcement 2: Added "for potential copyright issues by comparing the output to competitor marketing materials" to clarify what sales managers should be looking for during their review.
    *   Enforcement 5: Added "based on the severity and frequency of the violation, as well as the employee's history with the company" to clarify the criteria for disciplinary action.
    *   Removed the sentence about synthetic PII in Prohibited Uses section.
*   Removed item 3 from section 1, and items 2 and 3 from section 2, as they were not supported by the base facts.
