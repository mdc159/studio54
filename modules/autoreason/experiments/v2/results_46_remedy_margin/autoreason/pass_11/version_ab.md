...
|------|-------------|-------------------|-------------------|
| Engineering Lead | Lead | Architecture decisions, vendor relationships, executive escalation | E1 assumes authority for operational decisions during absences; see 7.4 |
...
- SMS use-case expansion beyond auth and P0 alerts
...
**On the SMS scope framing:** SMS is operationally live from day one for auth and P0 alerts. The deferred item is expansion of SMS use cases beyond those two — not the channel itself. Describing SMS as "cut except for auth" is misleading because the operational burden of a live channel exists regardless of how its scope is labeled. The on-call page conditions for auth SMS failure rate in Section 7.3 reflect this: SMS is a monitored, live channel from launch.
...
**Unplanned unavailability:** If the lead becomes unavailable without designation, E1 assumes acting lead authority immediately. The executive sponsor is notified within 4 hours. If E1 is simultaneously unavailable, E4 assumes authority. The chain is Lead → E1 → E4. There is no scenario in which the team has no decision-maker.

**The honest framing:** The lead is the preferred P0 escalation path; E1 is the fallback with full authority. The lead is not guaranteed reachable at all times — that framing overstates the guarantee. The mitigation for this single point of failure is documentation: architecture decisions are written down in sufficient detail that E1 can execute them without the lead present. The mitigation is not redundancy of the person.

### 7.5 Executive Sponsor Role — Defined

The executive sponsor is referenced at multiple points in this document. Their role is defined here once.
...
- Removes organizational blockers the engineering team cannot remove (vendor contracts, budget approval, cross-team dependencies)
...
**Availability requirement:** The executive sponsor commits to a 4-hour response SLA for escalations during the 6-month window. If unavailable, they designate a proxy with equivalent authority in advance.

**If the sponsor is unavailable during a time-bounded escalation:** The engineering lead proceeds with the conservative option — the option that avoids irreversible decisions — without sponsor approval. Reversible decisions (adding capacity, pausing beta expansion) do not require sponsor approval. Irreversible decisions (architectural changes, vendor contract changes) wait for sponsor availability up to the escalation deadline, then default to the conservative option.
...
**If calibration fails** (standard deviation > 5%): E1 reports this to the engineering lead as a signal that DAU/MAU is too noisy for this trigger. The trigger is replaced with an absolute DAU count threshold.

**The coordinated capacity review:** Covers Redis node count, RDS write throughput and read replica need, and worker fleet size — evaluated together, not sequentially. E1 owns it; the engineering lead approves any recommendation requiring >20% infrastructure change. Target completion is one week.

If a Scenario A or B event occurs during the review window, the review is suspended and on-call takes priority. The review resumes after the event resolves; the completion window extends by the suspension duration, up to two additional weeks. If the review has not completed within three weeks of initiation, the engineering lead escalates to the executive sponsor — not because three weeks is a known failure threshold, but because a review that takes three weeks has either encountered a genuine architectural problem or has been repeatedly interrupted, both of which are executive-level concerns.

If the review window coincides with two or more engineers simultaneously carrying on-call load during a sustained event, the engineering lead explicitly pauses non-urgent delivery work for those engineers. The capacity review is not treated as background work.

### 1.2 The 17/Day Planning Ceiling

#### The Circularity Problem — And What Is Actually Done About It

The 17/day ceiling is circular: it is derived from benchmark-based per-tier rates multiplied by tier weights that cannot be validated before the infrastructure decisions that depend on them. Acknowledging the circularity and then using the 17/day figure as a decision gate input — as the prior version did — does not handle it honestly. The circular number simply propagates into downstream thresholds.

**What this version does instead:**

The 17/day figure is not used as a gate threshold. Decision gate thresholds in Section 1.2c are derived from the worst-modeled scenario (the 15/35/50 tier split, producing a 27/day ceiling) and expressed in absolute operational terms — notifications/sec at peak, queue depth, worker saturation — rather than as multiples of the 17/day baseline. This prevents the circularity from propagating into operational decisions.

Phase 1 infrastructure is provisioned at the 15/35/50 scenario ceiling at 60% capacity. The system is sized against the worst modeled case, not against a number that depends on the thing being sized. The cost is approximately $5–10K/month more than anchoring to the 17/day baseline would produce. That cost is accepted as the direct consequence of treating the circularity honestly.

The 17/day figure is retained below as a derivation artifact showing the calculation. It does not anchor any operational decision.

#### Derivation (Reference Only — Not a Sizing Anchor)
...
1. A reading at or below the planning ceiling does not confirm the ceiling is correct; it means there is no strong evidence of catastrophic error. The steady-state rate is the sparse-graph reading plus a suppression correction of unknown magnitude.
2. A reading above the ceiling despite sparsity suppression is a stronger alarm than the same multiple would imply in a mature network. Above-ceiling readings therefore trigger responses at lower absolute multiples than would apply in a dense graph.
...
### 1.2a Queue Equilibrium Under Spike Conditions

#### The Overlap Problem — Resolved

With a 15-minute batch cycle and drain times that exceed the cycle length, multiple batches accumulate simultaneously before the first finishes draining. This section establishes whether the queue reaches stable equilibrium or grows without bound.
