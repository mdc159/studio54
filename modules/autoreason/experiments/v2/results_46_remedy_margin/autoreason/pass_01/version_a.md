...
**Provider:** SendGrid. At 4M/day, we're in their Pro tier (~$1,500/month). We considered AWS SES ($0.10/1K = $400/day cheaper) but SendGrid's deliverability tooling, suppression list management, and webhook reliability are worth the premium for a team of 4.
...
```python
# Pseudocode for digest aggregation
...
- Password reset (if no email in 5 minutes)
...
def dequeue_batch(worker_id: str, batch_size: int = 50) -> list[Notification]:
    # Atomic pop of top N items
    pipe = redis.pipeline()
    pipe.zrange("notification_queue", 0, batch_size - 1, withscores=False)
    pipe.zremrangebyrank("notification_queue", 0, batch_size - 1)
    results, _ = pipe.execute()
    return [Notification.parse(redis.get(f"ntf:{id}")) for id in results]
```

**Tradeoff:** Redis Sorted Set is not a perfect priority queue — concurrent workers can create contention on the zrange+zrem operation. We use a Lua script to make it atomic. At our throughput (1,750/sec peak), this is fine. At 10× this scale, we'd move to a dedicated queue system like Amazon SQS with priority queues or Apache Pulsar.

### 4.3 Batching Logic

**Aggregation (same notification type, same user, short window):**

"Alice liked your photo" followed 30 seconds later by "Bob liked your photo" should become "Alice, Bob, and 3 others liked your photo."

```python
AGGREGATION_WINDOWS = {
    "like": 300,      # 5 minutes
    "follow": 600,    # 10 minutes
    "comment": 60,    # 1 minute (comments are more personal)
    "mention": 0,     # Never aggregate mentions
    "message": 0,     # Never aggregate DMs
}

def route_notification(event: NotificationEvent):
    window = AGGREGATION_WINDOWS.get(event.type, 0)
    
    if window > 0:
        agg_key = f"agg:{event.user_id}:{event.type}:{event.entity_id}"
        existing = redis.get(agg_key)
        
        if existing:
            # Add to existing aggregation, reset TTL