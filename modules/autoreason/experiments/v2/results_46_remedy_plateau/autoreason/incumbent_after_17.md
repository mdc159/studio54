...
### Revision 2 — Addressing Critic Findings

*Changes from Revision 1 are marked with* ⟦R⟧ *at the section heading where substantive changes occur.*
...
**SMS SLA interaction:** The 60-second SMS p99 target (§3.4) is affected by this choice. See §0.3 and §3.4 for the full analysis. The short version: SMS can meet its SLA under Sentinel if it bypasses the standard queue path. If the engineering lead selects Sentinel, the SMS direct-dispatch architecture in §3.4 is mandatory, not optional.

### 0.3 Interaction Analysis ⟦R⟧

The key interaction question: *Is priority isolation required during infrastructure failover, or is degradation acceptable?*

⟦R⟧ The 8× spike row has been added. This is the highest-risk scenario identified in §2.4 and was absent from the prior version of this table. The Redis failover + 8× spike column reflects the compounded worst case.
...
⟦R⟧ **Corrections from Revision 1:**
- Worker capacity range corrected from 2,200–4,400/sec to 1,300–3,500/sec. The prior figure used an inconsistent efficiency assumption. See §1.2 for full derivation.
- Planning figure revised from 3,000/sec to 2,000/sec. The prior figure was described as "conservative" but sat in the upper third of the derived range — an inconsistency. 2,000/sec sits at approximately the 50th percentile of the range and is genuinely conservative relative to the planning scenario. See §1.2 for the reconciliation.
- Off-hours 3× spike delay corrected from 28 minutes to 49 minutes. The prior figure was wrong: 3× off-hours load (909/sec) exceeds the primetime sustained rate (840/sec) in absolute terms, so queue dynamics are similar or worse, not better. See §1.2 for the corrected rate analysis.
- "May not drain" for the 8× spike scenario is now defined operationally. A queue depth of 500K messages triggers the operational response procedure in §2.4.
...
#### Email Volume Construction ⟦R⟧

Email volume is derived from the session-distribution model. The prior version of this document used a "6.3 events/day per eligible user" figure that was not independently derived — it was reverse-engineered from the 8M/day target and therefore circular. The construction below derives the figure from first principles.

**Step 1: Email-eligible population.** 10M MAU × 60% with verified email = 6M email-eligible users.

**Step 2: Reachable fraction.** Of the 6M email-eligible users, those in active session at routing time receive push or in-