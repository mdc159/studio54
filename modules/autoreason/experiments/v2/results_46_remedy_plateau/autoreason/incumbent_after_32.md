...
**All sections are provisional until the two open decisions in §9 are resolved.** The DAU/MAU ratio is the first multiplier in every throughput, sizing, and cost calculation in this document — including WebSocket server counts, Redis cluster sizing, SQS queue configuration, and SMS cost projections. No infrastructure should be provisioned from any section until §9 decisions are resolved.

A previous draft incorrectly declared certain sections "implementation-ready" while the DAU/MAU ratio remained unvalidated. That declaration has been removed. The dependency is called out explicitly in each affected section.

Target resolution: 14 days from issue date.
...
**Critical distinction:** The summary table distinguishes two uses of "peak" that are frequently conflated. The geographic concentration peak (~1,040/sec) is the expected sustained maximum during evening traffic. The burst headroom ceiling (~1,560/sec) is the infrastructure provisioning upper bound — the number used to size queues and autoscaling limits. Do not provision infrastructure to the geographic peak and assume burst is covered.
...
The baseline uses **2.0×** (US-centric). Infrastructure is provisioned to the burst ceiling of 3.0× average (~1,560/sec), not the geographic peak. If the product is genuinely global, infrastructure will be slightly over-provisioned — acceptable. If more concentrated than assumed, the numbers hold.
...
**Duplicate delivery for Critical notifications is a product decision, not a default.** Parallel dispatch means a user may receive the same security alert via push, email, and SMS simultaneously. For a 2FA code or login alert, this creates multiple simultaneous interruptions. Jordan Rivera (Product Lead) has reviewed and accepted this behavior for the following reasons, confirmed in product review on [DATE — to be filled before publication]:
...
**Mitigation for duplicate UX:** Notification copy for Critical events is written to be idempotent across channels. The email subject, push title, and SMS body all use the same action-oriented phrasing ("Action required: new device login detected") so that receiving multiple copies is alarming in the correct direction — the user understands something happened — rather than confusing. This copy convention must be enforced in the `event_type_registry` template fields.
...
**SMS cost note:** At 10M MAU, if 1% of users receive one SMS per month, that is 100,000 messages — $5,000–7,000/month. SMS is triggered by security events rather than daily activity, so this figure does not scale directly with DAU/MAU ratio, but the opted-in user base eligible for SMS events does grow with the overall user base. SMS is restricted to security events for opted-in users. This is a deliberate product constraint, not a technical limitation.
...
Device tokens are soft-deleted after 90 days without a refresh. This threshold interacts with a second invalidation path in a non-obvious way that requires explanation.

FCM and APNs return an invalid-token error when a push is attempted to an uninstalled app. The error-handling logic in §6 triggers a hard-delete on receipt of this error. This means uninstalled apps are caught at the point of attempted delivery regardless of the 90-day window. Given this, why soft-delete at 90 days at all?

Two reasons:

1. **Suppressing unnecessary push attempts.** A token that has not refreshed in 90 days is likely stale — the user may have switched devices, reinstalled with a new token, or stopped using the app. Attempting push to these tokens generates FCM/APNs API calls that will mostly fail and return errors. Processing those error responses consumes worker capacity. The 90-day soft-delete reduces this noise before it reaches the provider.
...
**The 90-day threshold is configurable** in the `system_config` table. The correct value depends on session-length distribution data that does not yet exist. If analytics shows that 20% of users have gaps of 60–90 days between sessions but return reliably, raise the threshold to 120 days. If the median gap for churned users is 30 days, lower it. Revisit at the 3-month post-launch review.
...
**Redis pub/sub fan-out behavior:** Sticky sessions are disabled because connection state is in Redis, not in local server memory. This creates a specific fan-out pattern that must be understood before accepting it.

When a notification is published to `notifications:{user_id}`, every WebSocket server instance subscribed to that channel receives the message. Without sticky sessions, a given user's connection lands on one of N server instances. The other N-1 instances receive the pub/sub message, find no matching local connection, and discard it. This is wasted work, not a correctness problem — the one server holding the connection delivers the message correctly.

**Why we accept this tradeoff:** The alternative — sticky sessions — creates a worse failure mode. If the server holding a user's sticky connection fails, the connection must be re-established and re-routed, requiring either a shared session registry (the same Redis already in use) or a long reconnect window. At 62,500 concurrent connections across 6–8 instances, the fan-out overhead is modest: each published message is received by 7 instances, 6 of which perform a hash lookup and discard. At the expected pub/sub rate of ~1,040 events/second during peak, this is ~6,240 wasted lookups/second across the fleet — negligible. If instance count grows significantly beyond 20 instances, revisit this decision and consider a routing layer that tracks which server holds each user's connection.

**Redis pub/sub durability:** Redis pub/sub is not persistent. A message published when no subscriber is connected is lost at the pub/sub layer. This is acceptable because the notification is written to the database before publication. On reconnect, the client sends its last-seen notification ID and the server returns all unread notifications with a higher ID — a simple range query, not a complex sync protocol. There is no message loss, only a delivery mode change from real-time to on-reconnect.

**WebSocket scaling:** At 3M DAU (30% DAU/MAU assumed — *provisional until §9 resolved*) with an average session length of 20 minutes distributed across a 16-hour active window, concurrent WebSocket connections peak at approximately 3M × (20/960) ≈ 62,500. This calculation changes proportionally with the DAU/MAU ratio: at 60% DAU/MAU, peak concurrent connections approximately double to ~125,000.

**WebSocket server sizing:** At 62,500 concurrent connections, a t3.medium instance handles ~10,000–15,000 connections comfortably. Plan for 6–8 instances behind a load balancer with sticky sessions disabled. Autoscale on connection count at 70% capacity per instance.
...
**The deliverability tradeoff is real and must be managed actively.** This is not a set