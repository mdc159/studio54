...
Three decisions require explicit sign-off. Decisions A and B interact; the interaction analysis is in §0.3.
...
- **Option A:** Dedicated high-priority worker pool. Separate process group, separate Redis consumer group. Hard isolation between priority tiers during normal operation; partial isolation during Redis failover because high-priority workers can be configured to retry Redis more aggressively before falling back.
...
This default requires active acknowledgment, not silence. Both decision owners must confirm in writing that they have read this section and accept Option C, or nominate an alternative. An unread or unacknowledged 14-day window does not constitute sign-off. If no response is received, the engineering lead will escalate to both parties directly before the window closes.
...
**Default if no explicit sign-off within 14 days: Sentinel.** Same acknowledgment requirement as Decision A.
...
| Worker capacity (100 workers) | 2,200–4,400/sec | Bounded range | §1.2 |
| Planning figure (100 workers) | 3,000/sec | Conservative bound; see §1.2 | §1.2 |
...
| Standard-priority delay, primetime 8× spike | ~4 hours; may not drain | Scenario analysis; §2.4 | §2.4 |
...
*Validation commitment:* Measure from beta cohort event logs. Owner: product lead. Pass criterion: 5–12/day. Fail criterion: outside that range.
...
*Validation commitment:* Measure from beta permission-prompt flow. Owner: mobile engineering lead. Pass criterion: 40–65%. Fail criterion: outside that range.
...
The email routing model requires an estimate of how many MAU have no active session at notification time. We assume 40% of MAU are inactive at any given routing moment during primetime. This determines email volume. See §1.1 (Email Volume) below for the full construction.
...
Email volume is not a standalone figure — it is derived from the session-distribution model. Here is the construction:

- 10M MAU total; assume 60% have verified email addresses = 6M email-eligible users.
- Of those 6M, assume 30% are in active session at any given routing moment during primetime (Assumption 4: 40% inactive, but 30% of the *email-eligible* subset are active, meaning they receive push or in-app instead). This leaves 70% of email-eligible users reachable by email at any given moment.
...
- Daily email-eligible users who receive at least one email-routed notification: 6M × 70% reachable × 30% event fraction × (some events per day per user) ≈ 8M/day.

The full derivation: 6M eligible × 70% reachable fraction × 30% high-value event fraction × ~6.3 events/day per eligible user = approximately 8M/day. The 6.3 events/day figure is derived from 8 events/day per DAU scaled to the email-eligible population, which skews slightly less active.

**This is Assumption 4 and 5 combined.** The 78% figure used in the prior version of this document was a collapsed intermediate that obscured the construction. The construction above shows all steps. The 8M/day figure carries the same ±50% uncertainty as the push figure — it rests on the same unvalidated inputs. Infrastructure is not separately sized for email volume because email dispatch is rate-limited by provider API constraints (§3.3), not by internal worker capacity.
...
| SMS | Auth events only | Event-limited | ~50K | See §3.4 for SLA and volume derivation |
...
### 1.2 Rate Derivation and Worker Capacity

#### Sustained Rate

34.55M/day is not uniformly distributed. Primetime (6pm–11pm local, approximately 5 hours) carries a disproportionate share of daily volume. Based on the session-distribution assumption, we model primetime as carrying 40% of daily volume in 5 hours, off-hours as carrying 60% in 19 hours.

Primetime sustained rate: 34.55M × 0.40 / (5 × 3,600) = 13.82M / 18,000 = **767/sec**, rounded to ~840/sec after applying a 10% headroom factor for within-hour variability.

Off-hours sustained rate: 34.55M × 0.60 / (19 × 3,600) = 20.73M / 68,400 = **303/sec**.

#### Worker Capacity

Worker throughput depends on external API latency, which we do not control. The range is:

- **Optimistic (p50 latency ~20ms):** 1 worker × (1/0.020) × 0.70 efficiency = ~35/sec per worker. But this is not the planning figure.
- **Conservative (p99 latency ~45ms):** 1 worker × (1/0.045) × 0.60 efficiency = ~13/sec per worker.
- **Planning figure: 30/sec per worker.** This is not the midpoint of the range — it is a conservative bound that sits closer to the p75 latency estimate than the p50, because during spikes the external APIs experience elevated latency concurrently with our elevated volume. Using the p50 figure as a planning assumption would underestimate spike-time capacity degradation.

At 100 workers: planning capacity = 3,000/sec. Range = 1,300–3,500/sec. The executive summary table states the range as 2,200–4,400/sec, which used a different efficiency assumption. **Correction:** The range is 1,300–3,500/sec at 100 workers using the derivation above. The planning figure of 3,000/sec sits in the upper third of this range — it is conservative in the sense of reflecting favorable but not best-case conditions. This will be measured at the pre-launch validation gate; if measured throughput falls below 2,000/sec, add 50 workers before launch.

**Why 3,000/sec is used despite being in the upper third of the range:** At sustained primetime load of ~840/sec, even the conservative lower bound of 1,300/sec provides 1.5× headroom. The planning figure matters most for spike analysis, where we need to know how much queue accumulates