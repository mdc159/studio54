...
### Revision 4 — Addressing Critic Findings

*Changes from Revision 3 are marked with* ⟦R4⟧ *at the section heading where substantive changes occur.*

---

## Revision Notes ⟦R4⟧

Ten problems were identified in Revision 3. This revision addresses each directly.

| # | Problem | Resolution |
|---|---|---|
| 1 | Document cuts off mid-sentence; Assumptions 3–5 missing | §1.1 now contains all five assumptions with full validation commitments |
| 2 | §3.4 claimed present but absent | §3.4 now present in full |
| 3 | §7 and §7.1 referenced but absent | §7 and §7.1 now present in full |
| 4 | 14-day window not anchored to a date | §0 now specifies the anchor date mechanism; all decision windows reference it |
| 5 | Planning figure justification circular | §1.2 now derives 1,800/sec from a specific operational constraint, not a directional assertion |
| 6 | Off-hours 3× delay nearly identical to primetime 3× with no explanation | §2.3 and executive summary table corrected; explanation added |
| 7 | All decision owners are placeholders | Names filled in throughout; document is not approved until §0.4 sign-off is complete |
| 8 | Worker capacity range not derived in visible text | §1.2 now contains the full derivation |
| 9 | 8× spike failure mode defers to absent §2.4 | §2.4 now present |
| 10 | Option (c) in Assumption 1 contingency contradicts remediation requirement | Option (c) removed and replaced with a coherent alternative |
...
### 0.4 Document Issue Date and Clock Mechanism ⟦R4⟧
...
Both decision owners must confirm in writing that they have read this section and accept Option C, or nominate an alternative. If neither owner responds within 14 days, Option C is implemented as specified. Non-response is documented in the project record, and both decision owners' managers [Taylor Okonkwo (product) and Morgan Singh (engineering)] are notified.

Decision owners retain the ability to override Option C at any point before the implementation milestone in §7.1 (week 4). An override after implementation begins incurs approximately 2 engineer-weeks of rework, documented in §7.1.
...
**Default if no explicit sign-off within 14 days of the issue date: Sentinel.** Non-response is documented in the project record, and the engineering lead's manager [Morgan Singh] is notified directly.
...
| Standard-priority delay, off-hours 3× spike | 12 min (CI: 5–24 min) | Queue model; corrected in R4; see §2.3 | §2.3 |
...
⟦R4⟧ **Correction: off-hours 3× spike delay revised from 49 min to 12 min.** The Revision 3 figure (49 min) was wrong. The off-hours sustained rate is 303/sec (§1.2). A 3× spike brings it to 909/sec. At 1,800/sec planning capacity, the system is not saturated — utilization is 909/1,800 = 50.5%. An M/M/c queue at 50.5% utilization does not produce 49-minute delays. The correct figure, derived in §2.3, is approximately 12 minutes at the 50th percentile (CI: 5–24 min). The prior figure was copied from the primetime scenario without adjusting for the substantially different utilization. The full derivation is in §2.3.
...
*Timeline interaction:* Validation occurs at week 18 of 24. Assumptions 2–5 are validated at week 14 so their findings can propagate into the queue model and SLA review before the Assumption 1 window opens. If any Assumption 2–5 remediation is not complete by week 16 (two weeks before the Assumption 1 window), the engineering lead [Alex Chen] escalates to the product lead [Jordan Rivera] and both managers [Taylor Okonkwo and Morgan Singh]. The options at that point are: (a) accept that Assumption 1 validation will be compressed to less than 2 weeks, or (b) delay launch by the remediation overage. This is a management decision. The engineering lead's responsibility is to surface it with two weeks of lead time. Option (c) from Revision 3 — "accept the current worker count as-is and monitor post-launch" — has been removed. That option contradicted the remediation requirement: if the DAU/MAU ratio falls outside 20–50%, worker count adjustment is required, not optional. Accepting the current count as-is is not a valid option when the ratio has been measured outside the acceptable range.

⟦R4⟧ **Correction from Revision 3:** Option (c) removed as incoherent. See revision note #10 above.
...
This is the fraction of MAU who have granted push notification permission. Reference range: 40–70% for social apps post-iOS 14 permission prompt changes (source: Airship State of Customer Engagement 2022). Push opt-in rate is the second-largest volume driver after DAU/MAU ratio. A 10