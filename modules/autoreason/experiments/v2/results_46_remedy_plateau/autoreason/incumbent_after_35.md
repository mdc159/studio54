...
The 3.0× shorthand is only correct for a US-only (2.0×) geographic concentration. If §9 resolution produces a different geographic distribution, the burst ceiling must be recalculated using the appropriate multiplier from §1.1.4.
...
The 99.5% figure is derived from component SLAs and failure mode analysis, not chosen arbitrarily.

| Component | SLA | Notes |
|---|---|---|
| SQS (standard queues) | 99.9% | ~8.8 hours annualized downtime |
| ElastiCache (Multi-AZ) | 99.9% | ~8.8 hours annualized downtime |
| RDS (Multi