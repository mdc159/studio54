...
2. **Parallel dispatch sign-off (§2.1):** The decision to use parallel multi-channel dispatch for Critical notifications carries real cost and UX consequences — including SMS cost implications not previously captured in cost projections. This requires explicit sign-off from Jordan Rivera (Product Lead) before the Critical notification path goes to production. The sign-off process is defined in §10. The placeholder in §2.1 must be replaced with a confirmed approval reference before production deployment.
...
The 3.0× multiplier is the burst headroom factor applied to average throughput. It already accounts for geographic concentration and is not multiplied by the geographic peak multiplier separately. The geographic peak figures in §1.1 and §1.1.4 describe the expected sustained maximum and are used for monitoring alert thresholds — not provisioning. Applying both multipliers would compound them incorrectly.

To derive the geographic peak for monitoring and alerting purposes only:
...
1. Security events require acknowledged delivery, not just attempted delivery. Redundancy is the correct behavior for "unrecognized device login" — a user who receives the same security alert via push, email, and SMS simultaneously understands that something happened and that it is urgent.
...
3. Sequential fallback with async push confirmation requires stateful per-notification tracking infrastructure — persistent state, a timeout mechanism, a reconciliation worker, and careful handling of the race between receipt arrival and timeout expiry — that is disproportionate to this team size. Parallel dispatch sidesteps this problem entirely. See §2.1.1 for detail.

**SMS cost implication of parallel dispatch — requires explicit accounting:**

Parallel dispatch means every user registered for SMS receives an SMS for every Critical event, regardless of whether push or email delivered successfully. This is materially different from a flat per-user estimate and must be modeled explicitly.
