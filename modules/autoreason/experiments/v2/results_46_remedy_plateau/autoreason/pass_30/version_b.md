...
This is the revised authoritative version. It supersedes all prior versions. Two open decisions (§9) require sign-off within 14 days of issue date. All other sections are final and implementation-ready.
...
- **Peak daily notification volume:** ~45M events
- **Peak hourly volume:** ~8M events (assuming 2.5× average during evening peak, 6–10 PM local time)
- **Average sustained throughput:** ~520 events/second
- **Peak throughput:** ~2,200 events/second
...
Four channels are supported. The selection logic is: attempt channels in preference order, stop when delivery is confirmed, unless the event type requires multi-channel delivery (§3.1).
...
**SMS cost note:** At 10M MAU, even 1% of users receiving one SMS per month is 100,000 messages, or $5,000–7,000/month. SMS is restricted to security events (password reset, login from new device, 2FA) to control this cost. This is a deliberate product constraint, not a technical limitation.

### 2.2 Push Notifications (FCM / APNs)

**Provider:** Firebase Cloud Messaging for Android, Apple Push Notification service for iOS. Both are managed services. A 4-person team should not operate push infrastructure.
...
**Tradeoff:** We do not implement read receipts at the push layer (only at the in-app layer). Push tells us the message reached the device; it does not tell us the user saw it. This is acceptable — the in-app layer handles read state.

### 2.3 In-App Notifications

**Mechanism:** WebSocket connection maintained while user is in active session. Notification center (bell icon) pulls from a server-side inbox on load and on reconnect.

**Storage:** In-app notifications are stored in the `notifications` table for 90 days, then archived. The inbox query is: unread notifications for this user, ordered by priority then timestamp, limit 50.
...
**Unsubscribe:** One-click unsubscribe link in every email, per CAN-SPAM/GDPR requirements. Unsubscribe events from SES callbacks update user preferences within 60 seconds. Suppression list maintained in SES; we do not re-add users who have unsubscribed from a category.

**HTML templates:** Stored in S3, rendered server-side using a simple templating library (Handlebars or Jinja2). No inline CSS frameworks — keep templates simple enough that one engineer can maintain them.

### 2.5 SMS

**Provider:** Twilio. Twilio is more expensive than alternatives (Vonage, AWS SNS for SMS) but has the best deliverability documentation, the most mature Node/Python/Go SDKs, and the easiest compliance handling for 10DLC registration in the US. The cost premium is justified for a security-critical channel where a 4-person team cannot afford deliverability debugging.
...
We use a two-tier schema: **Critical** and **Standard**. A three-tier schema adds implementation complexity (separate queue configurations, additional routing logic, more conditional branches in the worker) that is not justified at 10M MAU with a 4-person team. The two-tier schema handles all real use cases cleanly.
...
*Delivery target: < 5 seconds. All appropriate channels attempted in parallel. No batching.*
...
**Why not three tiers?** A "High" middle tier sounds useful in the abstract but creates classification disputes at the product level ("is a reply to my comment High or Standard?") and requires a third queue with its own consumer configuration. The real distinction that matters operationally is: "wake the user up if necessary" (Critical) versus "deliver efficiently when convenient" (Standard). Two tiers map cleanly onto that distinction.

**Runtime override:** Event-type classification is set at registration in the `event_type_registry` table. A privileged internal service account can override classification for a specific event instance, with the override logged to the audit table. Overrides are restricted to a defined allowlist of event types (e.g., a viral post that crosses a threshold might be upgraded). Override attempts outside the allowlist are rejected with a 403 and logged. This provides operational flexibility without opening arbitrary runtime reclassification.
...
- The batch window would push delivery past the user's next active period (estimated from historical session data — simple heuristic, not ML)
- The notification is the first Standard notification in > 4 hours (deliver immediately to maintain engagement signal)

**Quiet hours:** Default quiet hours are 10 PM–8 AM in the user's local timezone. Users can adjust or disable. During quiet hours, Standard notifications are held in the batch queue and released at the quiet-hours boundary. Critical notifications ignore quiet hours.
...
Caps are enforced by a Redis counter with a 24-hour TTL, reset at midnight UTC. Exceeding the cap silently drops the notification to in-app only and logs the suppression event. Silent drop is the right choice here: surfacing a "you've hit your notification limit" message creates a worse user experience than simply not sending the push.
...
Each user has a preference record with the following dimensions:
...
`per_type_overrides` allows granular control without a separate row per event type. The JSONB field is queried at delivery time; the full preference record is cached in Redis with a 60-second TTL. Preference changes are written to the database first, then the cache is invalidated (not updated — invalidation is simpler and avoids stale-write races).

**Why JSONB for overrides rather than a normalized table?** A normalized `user_event_type_preferences` table would have up to 10M × N rows where N is the number of event types. Most users don't customize per-type preferences. JSONB on the main preference row keeps the common case (no overrides) fast and the uncommon case (some overrides) still queryable. The tradeoff is that JSONB is harder to query in aggregate ("how many users have disabled push for likes?") — but that's an analytics query, not a hot path, and it can be handled with a GIN index or a periodic export.

### 4.2 Preference Propagation

Preference changes must be reflected in delivery within 60 seconds (success criterion from §1.3). The flow:
...
This is straightforward and meets the 60-second target easily. The only failure mode is if the queue consumer is down; in that case, the cache TTL (60 seconds) provides the fallback — the old preference will expire naturally within the TTL window.

### 4.3 Global Unsubscribe

A global unsubscribe (user opts out of all non-transactional notifications) is stored as a boolean flag on the user record, not in the preference table. This ensures it is checked at the earliest possible point in the delivery pipeline, before preference lookup, and cannot be accidentally bypassed by a bug in preference parsing. Transactional notifications (security events) are never suppressed by global unsubscribe.

### 4.4 Preference UI Principles

Not a backend concern, but worth stating to avoid misalignment with the product team:

- Preferences should be reachable in ≤ 3 taps from any notification
- "Unsubscribe from all" must be a single action, not a multi-step flow
- Preference changes must feel instant (optimistic UI update, background sync)
...
```
[Event Producers] → [Event Bus] → [Notification Worker] → [Channel Routers]
                                         ↓
                                  [Preference Cache]
                                         ↓
                                  [Delivery Queue]
...
All components run on AWS. The choice of AWS over GCP or Azure is not a strong technical preference; it reflects the assumption that the team has existing AWS familiarity. If the team is primarily GCP-fluent, substitute equivalent GCP services throughout.

### 5.2 Message Queue: Amazon SQS + SNS

**Choice:** SQS for delivery queues, SNS for fan-out from event bus to queue subscribers.

**Why not Kafka?** Kafka is the right choice if you need replay, complex stream processing, or multi-consumer fan-out at very high throughput. At 2,200 events/second peak, SQS handles this comfortably and requires zero operational overhead. Kafka on MSK adds ~$500–800/month and requires at least one engineer who knows how to operate it. At 4 engineers and 6 months, that cost is not justified. If the system grows to 100M MAU and requires replay or complex CEP, migrate to Kafka then.

**Queue structure:**
- `notifications-critical` — high-throughput, no batching, 1-minute visibility timeout
- `notifications-standard` — standard throughput, 15-minute visibility timeout, batching enabled
- `notifications-dlq-critical` — dead letter queue for critical failures, alarm on any message arrival
- `notifications-dlq-standard` — dead letter queue for standard failures, alarm on > 100