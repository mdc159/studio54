...
- Credential breach SMS uses its own tier (P-CB) rather than P0. P0 is optimized for sub-5-second latency on a per-event basis; P-CB is optimized for throughput across 10M recipients within a 4-hour window. These are different optimization targets requiring different infrastructure. The capacity model is in Section 2.6.
- The consent ledger is used to derive the eligible digest population, but cannot independently detect its own compromise. Section 1.3 specifies an independent pre-send enrollment-rate gate that reads from the raw event stream — not from the consent ledger — to close this circular dependency.
...
The fallback activates when SMS delivery is rate-limited or unavailable. The worst case is full SMS failure during a peak DAU period. At 3M DAU, assuming 8% of daily active users trigger an OTP event, that is 240K OTP events/day. If SMS is fully unavailable and 100% fall back to email, the fallback adds 240K emails/day to the transactional pool.

The 8% figure is conservative: it assumes every active user performs at least one OTP-requiring action per day, which overstates normal behavior. The 100% fallback rate assumes complete SMS failure. Both assumptions are deliberately pessimistic to establish a hard ceiling. At 50% SMS degradation, fallback adds ~120K emails/day — well within the 750K ceiling. Partial degradation scenarios do not require separate modeling.

**Planning bound:** 750K/day includes base transactional volume (~470K) plus worst-case fallback (240K) plus a ~10% buffer. The OTP fallback volume is not additive to this ceiling — it is already included in it.
...
The consent ledger is used both to derive the eligible send population and to set the anomaly threshold (2× ledger count). If the ledger is compromised by a bot-driven enrollment attack, both the send list and the threshold inflate together — the check becomes self-referential and fails silently.

**Independent pre-send enrollment-rate gate:** Before each digest job runs, a separate check reads the enrollment *rate* from the user registration event stream — not from the consent ledger. It computes the 7-day rolling average of new digest opt-ins per day from the raw event log and compares it to the current day's opt-in count. If the current day's count exceeds 5× the 7-day rolling average, the digest job is held and an alert fires to the on-call engineer for manual review before proceeding.

This gate does not read from the consent ledger. It reads from the immutable event stream that the consent ledger is derived from. A bot attack that inflates the consent ledger by creating fraudulent accounts will also produce an anomalous spike in the enrollment rate, which this gate catches independently.

**Limitation:** This gate catches acute volume anomalies. It does not catch sophisticated slow-burn attacks that stay below the 5× threshold over many days. Slow-burn fraud detection is the responsibility of upstream fraud controls noted as a hard dependency. The notification system's gate is a last-resort check against acute spikes, not a comprehensive fraud system.
