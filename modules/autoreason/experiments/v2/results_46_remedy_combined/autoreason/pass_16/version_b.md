...
**DAU:** Daily active users, modeled at 30% of MAU (3M) for load test parameterization. The 30% figure derives from industry benchmarks for social apps at scale (Facebook ~66%, Twitter ~40%, median social app ~25–35%); 30% is the conservative end of the median range. The sensitivity analysis is in Section 1.5.

The 30% assumption has a specific, limited role: it parameterizes the pre-launch load test. It does not directly set operational alarm thresholds. Operational thresholds are set from load test output, not from the 30% figure itself. However, if the 30% assumption is substantially wrong, the load test was run against the wrong parameters and the resulting baselines are miscalibrated from day one. Section 1.4 specifies a validation gate that must be passed before load-test-derived baselines are promoted to operational use.
...
**Decision rule and threshold justification:**

The ±5 point threshold is derived from cost and capacity bounds, not from internal consistency alone. The sizing ceiling (75% opt-in, 30 notifications/day) is provisioned at approximately 225M notifications/day. The base estimate (60% opt-in, 15 notifications/day) is approximately 90M/day. A ±5 point shift in iOS share produces a volume change of roughly 4–5% — within the headroom already provisioned between the base estimate and the sizing ceiling. A ±10 point shift produces a volume change of roughly 8–10%, which approaches the boundary of provisioned headroom and warrants recalibration. The threshold is set at ±5 points because shifts within that range are absorbed by existing headroom; shifts beyond it are not.

If observed iOS share is outside 55%–65%, the infrastructure lead produces a revised cost model within 2 business days. If the revised model requires a plan tier change, the decision is escalated to the engineering manager and product owner with a 24-hour decision window. The system can operate on the existing tier during this window — the aggressive scenario is already provisioned.
...
The consent ledger is both the sizing source and the anomaly detection boundary. This creates a known vulnerability: if the consent ledger is inflated by a bot-driven enrollment attack, the anomaly threshold is inflated alongside it, and the detection mechanism fails silently. This is not a problem the notification system can solve internally — the notification system cannot be the last line of defense against enrollment fraud. The external dependency is:

**Hard dependency (pre-launch):** The onboarding flow must implement rate limiting and bot detection controls sufficient to prevent consent ledger inflation before the Day 7 digest send. This dependency is owned by the team responsible for the onboarding flow. If it is not confirmed as implemented before launch, the Day 7 digest send must be held pending manual review of the consent ledger count. The on-call engineer at 05:00 UTC on Day 7 is responsible for confirming that the dependency has been satisfied before releasing the digest job. This confirmation must be documented in the incident log regardless of outcome.

Using any WAND-derived ceiling as the Day 7 threshold would allow instrumentation errors to pass undetected. The consent ledger remains the correct source for Day 7, with the enrollment fraud dependency explicitly owned and tracked.
...
Before load-test-derived baselines are promoted to operational