...
- Digest email and transactional email use separate IP pools. A spam complaint spike from digest mail blacklisting the password-reset pool is a direct revenue impact. The IP pool warm-up schedule, failure handling, and operational ownership are in Section 3.3.
- Credential breach SMS uses its own tier (P-CB) rather than P0. The rationale is in Section 2.6. P0 is optimized for sub-5-second latency on a per-event basis; P-CB is optimized for throughput across 10M recipients within a target window. These are different optimization targets and require different infrastructure.
...
**DAU:** Daily active users, modeled at 30% of MAU (3M) for pre-launch sizing. The 30% figure derives from industry benchmarks for social apps at scale (Facebook ~66%, Twitter ~40%, median social app ~25–35%); 30% is the conservative end of the median range. The sensitivity analysis is in Section 1.5.

The 30% assumption has a specific, limited role: it parameterizes the pre-launch load test. It does not directly set operational alarm thresholds. All operational thresholds that depend on DAU are expressed as multiples of load-test-anchored baselines — not as absolute figures derived from the 30% assumption itself. However, if the assumption is substantially wrong, the load test was run against wrong parameters and the resulting baselines are miscalibrated from day one. Section 1.4 specifies a validation gate that must be passed before load-test-derived baselines are promoted to operational use.

**WAND (Weekly-Active-Not-Daily):** Users active in a given week but not on a given day. WAND is *not used for pre-launch infrastructure sizing*. It is not estimable before Week 4 of production. Pre-launch digest sizing uses the consent-ledger-derived eligible population method described in Section 1.3. WAND enters the model at Week 4 as a cross-check against consent-ledger counts, not as the primary input.
...
**Decision rule and threshold justification:** If observed iOS share is outside 55%–65%, the infrastructure lead produces a revised cost model within 2 business days. If the revised model requires a plan tier change, the decision is escalated to the engineering manager and product owner with a 24-hour decision window. The system can operate on the existing tier during this window — the aggressive scenario is already provisioned.

The ±5 point threshold is set at the boundary where volume changes exceed existing headroom. The sizing ceiling (75% opt-in, 30 notifications/day) is provisioned at approximately 225M notifications/day; the base estimate (60% opt-in, 15 notifications/day) is approximately 90M/day. A ±5 point shift in iOS share produces a volume change of roughly 4–5%, which is absorbed by provisioned headroom. A ±10 point shift produces roughly 8–10%, which approaches the headroom boundary and warrants recalibration. A ±10 point threshold would additionally place the 60/40→50/50 shift — a ~5% volume change — inside the no-action zone while claiming it is within planning tolerance, which is an internal inconsistency given the stated headroom model. The ±5 point threshold is therefore both cost-justified and internally consistent.
...
**Transactional and digest email use separate IP pools.** This is a deliverability decision with direct revenue impact and is not negotiable. Operational ownership, warm-up schedule, and failure handling are in Section 3.3.

**Transactional email:**

The figures below are pre-launch planning estimates used to parameterize the load test. They are not the source of operational alarm thresholds. Operational thresholds are set from load test baselines using the method in Section 1.4.
...
**Day 7 anomaly threshold and consent ledger integrity:**

The eligible population on Day 7 is fully enumerable from the consent ledger. The Day 7 anomaly threshold is set at 2× the consent-ledger-derived eligible population count, calculated at 05:00 UTC on Day 7 before the digest job runs.

The consent ledger is both the sizing source and the anomaly detection boundary. This creates a known vulnerability: if the consent ledger is inflated by a bot-driven enrollment attack, the anomaly threshold is inflated alongside it, and the detection mechanism fails silently. The notification system cannot be the last line of defense against enrollment fraud.

**Hard dependency (pre-launch):** The onboarding flow must implement rate limiting and bot detection controls sufficient to prevent consent ledger inflation before the Day 7 digest send. This dependency is owned by the team responsible for the onboarding flow. If it is not confirmed as implemented before launch, the Day 7 digest send must be held pending manual review of the consent ledger count. The on-call engineer at 05:00 UTC on Day 7 is responsible for confirming this dependency has been satisfied before releasing the digest job. This confirmation must be documented in the incident log regardless of outcome.

---

### 1.4 Alarm Baseline Initialization and DAU Assumption Validation

**The load test parameterization dependency:**

The load test is parameterized using the 30% DAU assumption (3M DAU). If the actual DAU/MAU ratio at launch is substantially different, the load test was run against wrong parameters and the baselines derived from it are miscalibrated. This risk is not eliminated by expressing thresholds as multiples of load-test baselines — it is deferred to the validation gate below.
