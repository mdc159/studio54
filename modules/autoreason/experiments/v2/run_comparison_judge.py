...
5 independent judges each see the original task prompt and two versions
with randomized labels (Version 1 / Version 2). They don't know which
method produced which. Each judge ranks them and explains why.
"""
...
JUDGE_SYSTEM = (
    "You are an independent evaluator. You have no authorship stake in either "
...
Two proposals have been produced independently to accomplish this task.

{proposals}

For each proposal, evaluate:
1. How well it accomplishes what the original task asks for
2. How specific, concrete, and actionable it is
3. Whether its assumptions and numbers are realistic
4. Whether it has unnecessary complexity or bloat that doesn't serve the task
5. Overall quality as a professional deliverable

Then pick a winner. Respond with your verdict in this exact format at the end:
...
async def main():
    ...
    for j in range(num_judges):
        # Randomize which is Version 1 vs Version 2
        ...
        judge_tasks.append(
            call_llm(
                JUDGE_SYSTEM,
                JUDGE_PROMPT.format(task_prompt=task_prompt, proposals=proposals),
    ...
    for j, (response, order) in enumerate(zip(responses, judge_orders)):
        ...
        print(f"  Judge {j+1}: picked Version {pick_num} = {pick_method} (order: 1={order['1']}, 2={order['2']})")
    ...
    (out_dir / "autoreason_vs_adversarial.json").write_text(
        json.dumps(summary, indent=2, ensure_ascii=False)
    )
    print(f"\nSaved to {out_dir / 'autoreason_vs_adversarial.json'}")
