...
*Changes made: Scope item 3 added to reference Slack AI features being disabled (fixes Problem 7). Permitted Uses item 2 (grace period) removed entirely (fixes Problems 4 and 6). Synthesis rationale section removed (fixes Problems 3 and 5). Prohibited Uses item 2 motivating-fact citation revised to remove the DPA flag and substitute the directly applicable base facts — Incident #2, Incident #3, and the surveyed unapproved usage rates (fixes Problem 2). These removals and the tightened citations bring the policy body within 500 words (fixes Problem 1).*