...
- **Problem 1 (word count):** Tightened all sections, eliminated redundant phrasing, compressed parentheticals.
- **Problem 2 (Permitted Uses incomplete for sales):** Added a numbered item explicitly permitting AI-assisted email drafting for sales staff subject to manager review.
- **Problem 3 (Slack prohibition motivation):** Replaced the invented DPA inference with the actual base fact: Slack AI features are currently disabled as an existing company control. The prohibition is now motivated by that control state, not a fabricated compliance finding.
- **Problem 4 ($50K budget unused):** Added a numbered item in Permitted Uses establishing that additional AI tools may be approved against the $50K annual budget allocation via a defined approval process.
- **Problem 5 (FedRAMP underused):** Incorporated FedRAMP substantively in the prohibition against unapproved tools, noting the Q3 authorization target creates a specific constraint on data handling.
- **Problem 6 (circular enforcement):** Replaced the managerless detection requirement with an anchor in the existing SOC2 access-review log audit, consistent with the no-new-tooling constraint.
- **Problem 7 (redundant negation):** Deleted the second sentence of former Permitted Use #1.
- **Problem 8 (80 seats / 120 engineers gap):** Added language in the Copilot permitted use item clarifying that unassigned engineers may not use Copilot until a seat is allocated by their Engineering Lead.
