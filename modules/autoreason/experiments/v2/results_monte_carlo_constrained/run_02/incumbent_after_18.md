...
- **Problem 1 (word count):** Compressed all sections, eliminating redundant phrasing throughout.
- **Problem 2 (Scope prose items):** Replaced Scope items 2–3 with definitional/applicability statements rather than background recitals.
- **Problem 3 (Prohibited Use #2 overbroad):** Narrowed the prohibition to sales staff using AI for outbound communications, matching the base fact.
- **Problem 4 (Slack omitted):** Added Slack (AI features disabled) explicitly to Permitted Uses.
- **Problem 5 ($50K budget decorative):** Added a numbered item making the $50K budget a hard cap constraint on tool approvals, not merely a passing reference.
- **Problem 6 (sales drafting assertion):** Removed the unsupported claim that no tool is "approved for sales drafting"; replaced with a neutral statement that additional tools require approval.
- **Problem 7 (access-log enforcement of external tools):** Removed the claim that IT detects unapproved external tool use via access logs; replaced with a manager-attestation mechanism using existing supervisory workflows, which requires no new tooling.
