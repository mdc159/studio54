...
3. Tools not listed under Permitted Uses are unapproved and subject to Prohibited Uses. No AI tool may be procured or expensed outside the $50K annual AI tooling budget and the Permitted Uses list.
...
- **Scope item 3:** X's budget constraint language is operationally meaningful and retained; Y's cleaner framing of unapproved tools is merged into the same item to avoid redundancy.
- **Prohibited Use #2:** X's explicit call-out of sales staff and the instruction to cease immediately is stronger enforcement signal than Y's generic language; retained from X.
- **Prohibited Use #6 (Slack AI):** X's reasoning is correct — Slack AI is already disabled by IT controls, so a prohibition on employees is redundant and potentially confusing. Item omitted per X's analysis.
- **Enforcement #2:** X's framing that approving a non-compliant PR is itself a violation is more enforceable than Y's passive "attestation" language; retained from X.
- **Enforcement #4:** X's addition of "by IT" designates a responsible actor, making the suspension step executable; retained from X.
- **Permitted Uses and Prohibited Uses #1, #3, #4, #5:** Both versions converged on identical language; retained as written.