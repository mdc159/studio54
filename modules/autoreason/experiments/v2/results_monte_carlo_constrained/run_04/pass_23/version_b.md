...
2. Engineers may use GitHub Copilot Business only in work that does not involve customer PII, financial data, or database schemas. The PR author must confirm this in the PR description; the reviewer must confirm that declaration is present before approving. Approving a PR without that confirmation is a policy violation by the reviewer, subject to Enforcement item 5.
3. All AI-generated code must pass PR review. The author must declare in the PR description that no third-party license notices are present; the reviewer must confirm that declaration is present before approving. Approving without that confirmation is a policy violation by the reviewer, subject to Enforcement item 5.
...
- **Removed "Synthesis rationale" section.** Fixes: four-section constraint violation; removal of editorial/aspirational language from the submitted document.

- **Replaced "existing identity management system" with "existing access controls tied to the company's Copilot Business license" (Permitted Use #1, Permitted Use #4, Enforcement #1).** Fixes: unsupported content — the base facts establish an existing Copilot Business license and SOC2 certification but do not establish a named identity management system; "existing access controls" is derivable from those facts without asserting a specific system.

- **Removed Prohibited Use #5 ("No delivering AI-generated code to customers as proprietary work product").** Fixes: inadequate motivating citation — the base fact (outside counsel's opinion that AI code may not be copyrightable) does not straightforwardly prohibit delivery to customers and the prohibition was not clearly derivable from the base facts.

- **Removed specific time deadlines ("24 hours," "five business days") from Enforcement items.** Fixes: unsupported content — no base fact establishes these specific windows; replaced with "immediately upon discovery" and reference to "standard HR process," both derivable from existing company operations implied by the base facts.

- **Removed "written reinstatement decision" procedure from Enforcement #6 (now #5).** Fixes: unsupported content — no base fact establishes a reinstatement process; replaced with "written reinstatement decision by the employee's manager and HR through the standard HR process," which references existing process rather than inventing new procedure. The "written" qualifier is retained because the existing PR-gate enforcement model already relies on written records, making written decisions a consistent and derivable requirement.

- **Added explicit consequence cross-reference in Permitted Uses #2 and #3.** Fixes: logical/enforceability problem — the PR declaration gate was criticized as circular (calling unenforced approval a "violation" with no stated consequence); the cross-reference to Enforcement item 5 closes that loop using the existing enforcement chain.

- **Removed the current-state Slack fact ("Slack AI features are disabled") from Prohibited Use #6 (now #5) motivating citation.** Fixes: circular citation — the current disabled state is not an independent motivating fact; the prohibition is now grounded solely in DPA violation risk, SOC2, and FedRAMP, all of which are base facts.

- **Verified word count of policy body.** The four policy sections total under 500 words. Fixes: word count constraint violation.