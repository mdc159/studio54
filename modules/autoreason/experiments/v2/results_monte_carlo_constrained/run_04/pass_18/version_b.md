...
1. **Scope item #3 removed** — fixes "Scope item #3 is redundant and functions as an uncited catch-all prohibition."

2. **Permitted Uses item #4 (budget approval workflow) moved to Enforcement** — fixes "Permitted Use #4 is procedural housekeeping, not a permitted use."

3. **Sales AI email use added to Permitted Uses** — fixes "Sales AI email use is never addressed in Permitted Uses," closing the gap for the 45% of sales staff using AI for email drafting, within the existing $50K budget approval process.

4. **Slack AI features added as an explicit Prohibited Use with motivating fact** — fixes "Slack AI features appear in Enforcement but not in Prohibited Uses."

5. **Enforcement item #3 monthly cadence removed; replaced with event-triggered verification** — fixes "Enforcement item #3 assumes an invented monthly cadence not derivable from any base fact."

6. **PR reviewer detection language revised** — fixes "Enforcement item #2 makes the PR reviewer personally liable for a detection task with no defined method." The reviewer is now required to confirm the author has declared no license notices are present, which is enforceable through existing PR review without new tooling.

7. **Word count reduced throughout** — fixes the 500-word constraint violation.
...
1. **GitHub Copilot Business** is the sole approved AI coding assistant. Seats are allocated to engineers by Engineering leadership, documented in the IT provisioning log, and capped at 80 seats.
