...
1. **No customer data in external AI services.** Inputting customer PII, financial data, or database schemas into any third-party AI tool is prohibited. *(Motivating facts: Incident #1: engineer pasted customer database schema into ChatGPT; outside counsel flagged DPA violation risk from inputting customer data into third-party AI services; GDPR obligations for EU customers; pending FedRAMP authorization Q3 target.)*
...
2. Pull request approval constitutes the reviewer's attestation that no third-party license notices are present in AI-generated code. The responsible engineer's manager executes an immediate revert on any merged PR subsequently found to contain third-party license notices.
