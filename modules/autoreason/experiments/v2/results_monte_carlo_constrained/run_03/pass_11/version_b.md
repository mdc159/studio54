...
**Problem 1 (word count):** Cut Scope items 3–4, collapse Prohibited Use rationale parentheticals, and tighten Enforcement prose.

**Problem 2 (Scope items 3–4 are non-actionable):** Replace item 3 with a scoped data definition and delete item 4 (survey statistics add no policy substance).

**Problem 3 ("likely violating" hedged language):** Replace with a firm statement of the factual basis without hedged qualifiers.

**Problem 4 (Enforcement gaps for Prohibited Uses 4 and 5):** Add explicit enforcement items for each.

**Problem 5 (engineers without seats unaddressed):** Add a Permitted Uses item stating that engineers without assigned seats are subject to Prohibited Use 5 and may not use any AI coding tool.

**Problem 6 (commit vs. merge mismatch):** Reframe Enforcement item 2 to trigger at PR review, not post-merge.

**Problem 7 (24-hour timeframe not derivable):** Remove the specific timeframe; retain the notification obligation without an invented deadline.

**Problem 8 ($50K budget as filler):** Remove the budget reference from Permitted Uses.
...
1. **No customer data in external AI services.** Inputting customer PII, financial data, or customer-identifiable information into any non-approved AI service is prohibited. *(Incident 1; outside counsel finding that doing so violates existing DPA terms; SOC2 Type II certification; GDPR obligations.)*
...
2. PR reviewers who identify AI-generated code lacking required license confirmation block the merge and escalate to engineering leadership, who log the incident before any merge proceeds. *(Prohibited Use 3)*
...
5. A first violation results in a written warning in the employee's HR file. A second violation results in termination. For violations of Prohibited Use 3, GitHub Copilot seat access is also revoked upon a second violation.