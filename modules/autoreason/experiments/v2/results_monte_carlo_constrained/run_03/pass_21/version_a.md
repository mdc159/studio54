...
1. **Removed the memo header block (TO/FROM/DATE/SUBJECT)** — fixes the word count constraint violation caused by the header pushing the document over 500 words.

2. **Deleted Scope item 3** — fixes the "prose-like filler" problem; the item was justificatory background, not a scoping rule, and its removal brings the document closer to the word limit.

3. **Added a Permitted Uses item 3 for sales AI use** — fixes the enforcement gap: 45% of sales staff use AI for email drafting per the base facts; the policy must address this use case rather than silently prohibiting it under Prohibition 4.

4. **Revised Enforcement item 4 to place accountability on the committing employee, not the reviewer** — fixes the unsupported assumption that a reviewer approved the bad merge; the base fact states only that an intern *committed* the code.

5. **Removed the "prior to any disciplinary determination" Legal pre-clearance requirement from Enforcement item 3** — fixes the invented procedural requirement not derivable from the base facts.

6. **Revised Prohibition 5's motivating citations** — removed SOC2 and GDPR as cited motivators; the only base fact is that the company has disabled Slack AI features. The citation now references only that base fact plus the outside counsel DPA finding, which is a direct base fact (not an inference about *why* Slack was disabled).
...
3. The company handles customer PII and financial data and holds SOC2 Type II certification, GDPR obligations for EU customers, and a pending FedRAMP authorization targeted for Q3.
...
2. **Sales staff** may use AI tools for internal email drafting only when the content is reviewed and confirmed accurate and original by the sender before transmission.
...
3. Violation of Prohibition 2 is reported by the employee's manager to Legal before any external remediation action is taken.
