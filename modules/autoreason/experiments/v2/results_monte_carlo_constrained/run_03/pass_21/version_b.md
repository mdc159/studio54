...
1. **Removed the editorial preamble entirely** — fixes Problems 1 and 6. The preamble is not policy content and inflated the word count.

2. **Deleted Scope item 3** — fixes Problem 2. Background context about compliance status is not a scoping rule.

3. **Revised Permitted Uses item 2 to remove the "recorded" requirement and the accuracy/originality confirmation language** — fixes Problem 3. No existing recording or review process for sales email is established in the base facts. The item now permits the use without inventing new procedural controls.

4. **Revised Prohibited Uses item 1 to remove SOC2, GDPR, and FedRAMP citations as motivators** — fixes Problem 4. The motivating facts are Incident 1 and the outside counsel DPA finding only.

5. **Revised Enforcement item 3 to remove the manager-to-Legal reporting chain and pre-remediation consultation requirement** — fixes Problem 5. No such process is derivable from the base facts.

6. **Revised Permitted Uses item 3 to remove the routing to Engineering or department leadership as approval authority** — fixes Problem 7. The base fact establishes only that a $50K budget is allocated; it does not establish an approval authority or process.
...
2. **Sales staff** may use AI tools for internal email drafting only when no customer data, competitor content, or unverified claims are included in the submitted prompt or output.
