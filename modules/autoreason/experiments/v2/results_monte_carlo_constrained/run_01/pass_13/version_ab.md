...
4. Additional AI tools require joint written sign-off from the heads of Engineering and Legal before any use begins, within the $50K annual AI tooling budget. Use without that sign-off is a policy violation.
...
4. **Slack AI features must remain disabled.** No employee may request or self-enable AI features within company Slack; the Slack administrator denies and logs all such requests. *[Basis: Company handles customer PII and financial data subject to DPA terms, GDPR obligations, and FedRAMP requirements; Slack AI features are administratively disabled per current configuration to prevent routing that data through unapproved third-party AI services.]*
