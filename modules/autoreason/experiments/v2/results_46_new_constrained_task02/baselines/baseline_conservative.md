...
| 15:10–17:45 | Team continues pursuing traffic-based hypotheses; replicas, connection pool limits, and caching layers investigated |
...
The production migration script executed a schema change that dropped the index on `transactions.created_at` (used by every paginated merchant query) without recreating it. Without the index, full table scans replaced indexed lookups, driving p99 query latency from 12ms to 8,400ms and causing upstream timeouts across the API.
...
**The wrong mental model held for 3 hours.** With Black Friday 2 days out, the team anchored on a traffic spike hypothesis. No one ran query-level diagnostics (`EXPLAIN ANALYZE`) until a senior engineer intervened 3 hours and 17 minutes into the incident.
