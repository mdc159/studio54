...
1. **Timeline — added outcome of scaling attempt** ("scaling does not resolve latency"): Fixes "timeline omits the explicit fact that scaling replicas did not help" and the related timeline editorializing/incompleteness problem.

2. **What Went Wrong — replaced "approximately 3 hours" with "3 hours 21 minutes"**: Fixes the hedging language violation (14:31 to 17:52 = 3h 21m).

3. **Summary — added 847 support tickets**: Fixes the missing required base fact.

4. **Remediation item 2 — replaced self-reported written confirmation with an independently observable artifact** (a passing migration run visible in the CI/CD pipeline dashboard): Fixes the falsifiability problem with item 2.

5. **Remediation item 4 — specified who runs the test, under what conditions, and where the result is recorded**: Fixes the weak falsifiability of item 4.

6. **Word count — trimmed throughout to reach ≤600 words**: Fixes the hard word count violation. No substantive facts were removed; only redundant phrasing was cut.
