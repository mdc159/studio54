...
| Opt-out compliance architecture | §2.4 | **End of Week 2** | Legal + E1 | Schema decisions in Month 1 made without this input; rework risk is high |
...
**On compliance specifically:** Section 2.4 presents two compliant architectures. Legal must select one by end of **Week 2**, not Week 3. The reason for the earlier deadline is direct: the compliance architecture choice has schema implications — synchronous preference check requires different query patterns and index structures than the cached path — and Month 1 engineering begins making those schema decisions in Week 3. A Week 3 decision arrives after those decisions are already in flight. If no decision is received by end of Week 2, the **conservative architecture** (synchronous preference check, no cache) activates as the default. This architecture is compliant under TCPA/CAN-SPAM/GDPR but carries a latency penalty documented in Section 2.4. The system will not launch with the cache-with-staleness path absent legal confirmation. This is a hard launch gate, not a recommendation.

**On Default A:** Default A is a fully specified operational procedure, not a named placeholder. The complete specification appears in §1.1 under "Reassessment Process." In brief: when stakeholders miss the Day 3 decision deadline during a reassessment, the Tier 3 worker poll interval increases from 500ms to 700ms (reducing Tier 3 throughput to ~71% of normal), and the email digest scheduler pauses new dispatch. This is implementable via configuration change and rolling restart in approximately 8 minutes, without a code deployment.
...
**The correct response to this uncertainty is monitoring, not a more precise projection.** The densification rate will be observable in production data within 4–6 weeks of launch, at which point a real rate replaces the range. Until then, the planning basis uses a conservative **1.0/DAU/day/quarter** (middle of the range).

**The correct monitoring signal is notifications/DAU/day, not total notification volume.** These are different problems with different responses:

- Total volume rises because DAU rises → add worker capacity proportionally
- Total volume rises because per-DAU rate is rising with flat DAU → evaluate batching changes first, then add capacity

Conflating these signals leads to the wrong intervention. The monitoring dashboard must expose both metrics as separate time-series. The per-DAU rate is computed daily and reported as a 7-day rolling average to reduce day-of-week noise.
...
**Volume headroom (growth buffer):** The difference between the planning basis (38.5M/day) and the capacity ceiling (48.1M/day) is approximately 25% in daily volume terms. This sounds substantial. It is not equivalent to a 25% throughput buffer.

**The reconciliation:** A 25% daily volume increase at the same concentration ratios produces a 25% peak rate increase: 2,540/sec × 1.25 = 3,175/sec. This exceeds the 2,650/sec worker ceiling by 20%. The 25% volume buffer is therefore the distance between the current planning basis and the point at which worker capacity must already have been added — not the point at which we begin the conversation.

**Deriving the trigger-to-ceiling relationship:** The worker ceiling is reached at the per-DAU rate that produces 2,650/sec peak. Working backward from the peak rate formula:
...
**This means the worker ceiling is breached at approximately 12.1/DAU/day under the High DAU scenario.** A trigger set at 13/DAU/day fires after the ceiling is already breached. This is the flaw in a per-DAU-rate trigger used in isolation.

**Corrected trigger design — throughput-based, not volume-based:**

The trigger fires on **observed peak throughput rate**, because that is what actually threatens the ceiling. The per-DAU metric is retained as a diagnostic signal to distinguish DAU growth from densification, but it is not the primary trigger.

| Threshold | Value | Meaning |
|-----------|-------|---------|
| Yellow alert | 7-day rolling average peak > **2,200/sec** | Acceleration signal; watch closely |
| Reassessment trigger | 3-day rolling peak > **2,400/sec** | Initiates formal reassessment process |
| Emergency threshold | Any single day peak > **2,550/sec** | Default A activates immediately, without waiting for reassessment |

**Reassessment process (triggered by 2,400/sec 3-day rolling average):**
...
Default A is not a placeholder. It is the following operational procedure, implementable without further design decisions and without a code deployment:
...
- Implementation: poll interval is a configuration value in the worker environment. Changing it requires a rolling restart of Tier 3 workers only — estimated 8 minutes.

**Default A effect:** Tier 3 is typically 35–45% of push volume at peak. Reducing Tier 3 throughput to 71% reduces effective peak demand by approximately 10–13%. At the 2,550/sec emergency threshold, Default A brings effective demand to approximately 2,220–2,295/sec — within the 2,650/sec ceiling. This provides operational breathing room while the longer-term option is implemented.

If option (b) is selected, Default A remains active until batching changes are deployed and validated. If option (a) is selected, Default A remains active until new capacity is confirmed operational.

---

#### Viral Spike Model

**The problem with "we'll monitor and respond":** The system launches in Month 2. If a significant viral event occurs in Month 3, the response time to add worker capacity is measured in hours to days. Monitoring identifies the problem; it does not prevent the delay. The spike model must specify degradation behavior, not just detection.
...
|------------|---------|-------------------|----------|
| Micro-spike | Single viral post, <5x normal rate | Minutes to 1 hour | Absorbed by queue depth; no action required |
...
Load shedding activates when push queue depth exceeds **450,000 messages**. Threshold derivation:

```
Normal peak rate: 2,540/sec
Target "3 minutes of backlog" at peak: 2,540 × 180 = 457,200 messages
Rounded to: 450,000 (slightly conservative; fires early)
```

Load shedding deactivates when queue depth falls below **200,000 messages** — not 225,000. The asymmetry is intentional. A symmetric threshold (activate at X, deactivate at X/2) on a bursty queue creates oscillation: shedding reduces inflow, queue drains toward the resume threshold, shedding deactivates, inflow resumes, queue refills, shedding reactivates. The 200,000 deactivation threshold creates a wider hysteresis band that prevents this oscillation while still resuming normal operation promptly after the spike passes.

**What happens automatically when load shedding activates (no human intervention required):**

1. **Batch-eligible notifications are held.** Like and comment notifications within their batching window are not dispatched until queue depth drops below 200,000. At peak, batching reduces push volume by 40–60% — the highest-leverage lever available.
2. **Re-engagement and digest emails are paused.** The email scheduler stops dispatching non-critical emails. Activity-triggered emails (direct user actions) continue unaffected.
3. **In-app notifications continue at full rate.** In-app delivery does not touch the push queue. Users active in the app see notifications normally.
4. **SMS is unaffected.** The SMS queue is isolated; it does not share workers with push.

**What we explicitly do not shed under any spike condition:**
- OTP and authentication SMS messages
- Direct message push notifications (Tier 1)
