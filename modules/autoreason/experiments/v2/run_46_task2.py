#!/usr/bin/env python3
"""
Task 2 with claude-sonnet-4-6: CoT judges, full autoreason + baselines + 7-judge panel.
Tests whether newer model changes the task-dependent result.
"""
...
async def main():
    ...
    out_dir = root / "results_46_task02"
    out_dir.mkdir(parents=True, exist_ok=True)

    print(f"Sonnet 4.6 + CoT — Task 2")
    print(f"Model: {MODEL}")
    print(f"{'='*60}\n")
    ...
    # Baselines
    print(f"  Running baselines (15 passes each)...")
    baseline_outputs = {}
    ...
    all_outputs = {"autoreason_46_cot": ar_output}
    ...
    print(f"\n  Results (sonnet-4-6, CoT judges):")
    for name, score in sorted_methods:
        print(f"    {name:25s} Borda={score:3d} 1st={first_place.get(name, 0)}")

    print(f"\n  Comparison:")
    print(f"    Sonnet 4 baseline judges:  c&r 22, autoreason 19")
    print(f"    Sonnet 4 CoT judges:       c&r 30, autoreason 25")

    (out_dir / "judge_results.json").write_text(json.dumps({"borda": borda, "first_place": first_place}, indent=2))
