...
- 5-minute batching windows for optimal balance of cost and responsiveness
- Up to 3 notifications per batch per user to avoid overwhelming users
...
class BatchProcessor:
    ...
    def process_batch(self, notifications):
        """Group notifications by user and channel, then summarize intelligently"""
        user_batches = defaultdict(lambda: defaultdict(list))
        
        for notif in notifications:
            user_batches[notif.user_id][notif.channel].append(notif)
        
        for user_id, channels in user_batches.items():
            for channel, user_notifications in channels.items():
                if len(user_notifications) > 1:
                    # Create summary for multiple notifications
                    summary = self.create_summary(user_notifications)
                    yield summary
                else:
                    yield user_notifications[0]
    
    def create_summary(self, notifications):
        """Convert multiple similar notifications into summaries"""
        if len(notifications) <= 2:
            return notifications[0]  # Don't summarize very small groups
        ...
        # Create summary based on most common type
        dominant_type = max(by_type.keys(), key=lambda k: len(by_type[k]))
        
        if dominant_type == 'like':
            return self.create_like_summary(by_type['like'])
        elif dominant_type == 'comment':
            return self.create_comment_summary(by_type['comment'])
        else:
            return self.create_generic_summary(notifications)
```

**Rationale**: 5-minute batching windows provide better responsiveness for social interactions while still achieving meaningful cost savings and reducing notification fatigue.
...
**Capacity Planning**:
- Peak load: 50,000 notifications/minute (assuming 5% DAU peak engagement - more realistic for social apps)
- Worker capacity: 200 notifications/second per instance
- Required instances: 5-8 during peak hours
...
- Processing latency (target: <5s for high priority, <5min for batched)
...
- Basic in-app notifications with simple API endpoint
...
- Priority-based processing with intelligent summarization
- Analytics and monitoring dashboard
- Circuit breakers and comprehensive retry logic
- Rate limiting implementation

### Month 6: Polish & Scale Testing
- Performance optimization and caching improvements
- Load testing with simulated 10M MAU traffic patterns
- Documentation, runbooks, and team knowledge transfer
...
**Decision**: 5 minutes for low-priority notifications with 3-notification limit
**Rationale**: Provides better responsiveness for social interactions while still achieving cost efficiency and reducing notification fatigue.
