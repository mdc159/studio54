...
- 5-minute batching windows for better user experience
- Up to 20 notifications per batch per user
- Intelligent deduplication and summarization

```python
class BatchProcessor:
    def __init__(self):
        self.batch_size = 20  # Per user limit - reduced for better UX
        self.batch_timeout = 300  # 5 minutes - shortened for social app
    ...
    def create_batch(self, notifications):
        ...
        return self.summarize_batches(batches)
    ...
    def summarize_batches(self, batches):
        ...
        for key, notifications in batches.items():
            if len(notifications) > 3:  # Lower threshold for social apps
                # Example: "John and 2 others liked your post"
                summarized[key] = self.create_summary(notifications)
            else:
                summarized[key] = notifications
        return summarized
```

**Rationale**: Shorter batching windows (5 min vs 15 min) and smaller batch sizes better suit social app engagement patterns where users expect more timely notifications about social interactions.
...
**Decision**: 5 minutes for low-priority notifications
**Rationale**: Social apps require more responsive notifications than enterprise apps. 5-minute windows provide good summarization opportunities while maintaining engagement.
