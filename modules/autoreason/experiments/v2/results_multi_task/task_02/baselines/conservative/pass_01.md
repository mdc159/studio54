...
- Up to 50 notifications per batch per user
- Intelligent deduplication and summarization

```python
class BatchProcessor:
    def __init__(self):
        self.batch_size = 50  # Per user limit
    ...
    def summarize_batches(self, batches):
        ...
        for key, notifications in batches.items():
            if len(notifications) > 5:
                # Example: "John and 4 others liked your post"
...
**Tradeoff**: Longer batching windows (15 min vs 5 min) reduce notification fatigue and infrastructure load, with acceptable latency for social interactions. Per-user batching limits prevent overwhelming individual users.
...
**Decision**: 15 minutes for low-priority vs 5 minutes
**Rationale**: Longer windows allow better summarization (e.g., "5 people liked your post" vs individual notifications), reducing notification fatigue while maintaining acceptable latency for social interactions.
