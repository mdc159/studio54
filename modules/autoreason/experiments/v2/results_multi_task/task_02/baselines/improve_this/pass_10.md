...
class PushDeliveryService {
  ...
  private async deliverToAllDevices(
    notification: PushNotificationPayload, 
    deliveryId: string
  ): Promise<PlatformDeliveryResult[]> {
    const results: PlatformDeliveryResult[] = [];
    const deliveryPromises: Promise<PlatformDeliveryResult>[] = [];
    
    for (const token of notification.recipient.deviceTokens) {
      const platformPromise = this.deliverToPlatform(notification, token, deliveryId);
      deliveryPromises.push(platformPromise);
    }
    
    // Execute all deliveries concurrently with timeout protection
