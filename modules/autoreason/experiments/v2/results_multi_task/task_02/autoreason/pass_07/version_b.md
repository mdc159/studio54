...
**Rationale**: PostgreSQL-primary storage with write-through caching ensures consistency while managed services reduce operational complexity. Queue-depth-aware processing prevents resource waste.

## 2. Device Token Management with Consistency Guarantees

**Fixes Problem #1: Cache invalidation race conditions**
...
class DeviceTokenManager:
    def __init__(self):
        ...
        )  # Fixes Problem #8: Connection pool exhaustion
        self.redis = Redis(host="redis-primary", port=6379)  # Fixes Problem #6: Simplified Redis
        self.cache_ttl = 300  # 5 minutes
        
    def store_token(self, user_id, device_id, platform, token):
        """Store token with write-through caching for consistency"""
        # Validate token format before storing (Fixes Problem #7: Token validation)
    ...
    def _validate_token_format(self, token, platform):
        """Validate FCM/APNs token format (Fixes Problem #7)"""
        if platform == 'android':
    ...
    def _fetch_tokens_from_db(self, user_id):
        """Fetch tokens from database with read replica support (Fixes Problem #6)"""
        query = """
...
## 3. Queue-Depth-Aware Processing Architecture

**Fixes Problem #2: Broken weighted fair queue implementation**
...
**Fixes Problem #4: Incorrect capacity calculations**

```python
class CapacityPlanner:
    def __init__(self):
        # Realistic SQS processing limits (Fixes Problem #4)
        ...
        # Realistic concurrent processing (Fixes Problem #4)
    ...
    def calculate_realistic_throughput(self):
        ...
        peak_hourly_messages = base_daily_messages * self.peak_hour_multiplier / 24
        ...
        return {
            ...
            "estimated_monthly_aws_cost": self._calculate_monthly_cost(base_daily_messages)
        }
        
    def _calculate_monthly_cost(self, daily_messages):
        """Realistic cost calculation including all services (Fixes Problem #9)"""
        monthly_messages = daily_messages * 30
        
        # SQS costs ($0.40 per million requests)
        sqs_cost = (monthly_messages / 1_000_000) * 0.40
        
        # FCM/APNs (free but rate limited)
        push_cost = 0
...
## 4. Simplified In-App Notifications

**Fixes Problem #3: Flawed delivery tracking and Problem #5: Database schema issues**

### 4.1 Partitioned Storage Without Complex Delivery Tracking
```sql
-- Partitioned table for scalability (Fixes Problem #5)
CREATE TABLE user_notifications (
    id BIGSERIAL,
    user_id BIGINT NOT NULL,
    content TEXT NOT NULL,  -- Simplified from JSONB (Fixes Problem #6)
...
-- Simple last-seen tracking per user (Fixes Problem #3)
...
class InAppNotificationService:
    ...
    def store_notification(self, user_id, content, notification_type):
        """Store notification with automatic cleanup (Fixes Problem #5)"""
        with self.db.transaction():
            ...
            # Efficient cleanup using partition-aware delete (Fixes Problem #5)
            cleanup_query = """
            ...
            """
            self.db.execute(cleanup_query, (user_id, user_id, self.max_notifications_per_user))
            
        return notification_id
        
    def get_user_notifications(self, user_id, since_timestamp=None, limit=50):
        """Get notifications using simple timestamp-based filtering (Fixes Problem #3)"""
