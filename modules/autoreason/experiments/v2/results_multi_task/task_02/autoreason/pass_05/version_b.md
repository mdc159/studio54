...
- **User Preference Service**: Denormalized preference management with cache-aside strategy
...
## 2. Device Token Management (Fixes Redis partitioning, cleanup, and disaster recovery problems)
...
class DeviceTokenManager:
    ...
    def _get_token_key(self, user_id, device_id):
        ...
        # Fixes: Redis Cluster partitioning hotspots
    ...
    def store_token(self, user_id, device_id, platform, token):
        ...
        # Fixes: Token cleanup and TTL issues
        ...
        # Sync backup to PostgreSQL with retry mechanism
        # Fixes: Disaster recovery backup reliability
        self.sync_token_to_postgres(user_id, device_id, platform, token)
    ...
    def get_user_tokens(self, user_id):
        ...
        # Fixes: Token cleanup removes invalid tokens properly
        if invalid_devices:
    ...
    def cleanup_invalid_tokens(self, failed_tokens):
        ...
        # Fixes: Token cleanup actually removes invalid tokens
    ...
    def restore_from_postgres(self):
        # Disaster recovery: rebuild Redis from PostgreSQL
        # Fixes: Actual disaster recovery implementation
        cursor = self.postgres_conn.cursor()
        cursor.execute("""
            SELECT user_id, device_id, platform, token, created_at 
            FROM device_tokens 
        ...
        for row in cursor.fetchall():
            user_id, device_id, platform, token, created_at = row
            
            # Calculate remaining TTL
            age_seconds = int(time.time()) - int(created_at.timestamp())
            remaining_ttl = max(0, (86400 * 90) - age_seconds)
            
            if remaining_ttl > 0:
                self.store_token(user_id, device_id, platform, token)
                restored_count += 1
                
        return restored_count
```

### 2.2 PostgreSQL Backup with Proper Recovery
```sql
-- Reliable backup table with proper indexing
-- Fixes: Disaster recovery backup structure
...
CREATE INDEX idx_device_tokens_created ON device_tokens(created_at);
```

## 3. Message Queue Architecture (Fixes SQS priority and volume calculation problems)

### 3.1 Separate Priority Queues
```python
class MessageQueueManager:
    def __init__(self):
        # Separate queues for true priority handling
        # Fixes: SQS message attributes don't provide priority
        self.queues = {
        ...
        # Realistic volume calculations accounting for spikes
        # Fixes: Volume calculations are dangerously wrong
    ...
    def send_notification(self, notification):
        ...
        # No delay seconds - handle batching in consumer
        # Fixes: Delay seconds breaks the priority system
    ...
    def process_messages_with_priority(self):
        ...
        # Fixes: Proper priority message processing
...
### 3.2 Realistic Capacity Planning with Spike Handling
```python
class QueueCapacityPlanner:
    def __init__(self):
        # Realistic social media usage patterns
        # Fixes: Account for viral content spikes
...
## 4. In-App Notifications with Proper Data Integrity (Fixes data corruption and race condition problems)

### 4.1 UUID-Based Storage with Atomic Operations
```python
class InAppNotificationService:
    ...
    def store_notification(self, user_id, notification):
        ...
        # Fixes: Redis sorted set scoring causes data loss
        notification_id = str(uuid.uuid4())
        notification_key = f"user_notifications:{user_id}"
        delivery_key = f"user_delivered:{user_id}"
        ...
        # Fixes: Use notification ID instead of JSON as sorted set member
    ...
    def get_user_notifications(self, user_id, since_timestamp=None, device_id=None):
        ...
        # Fixes: Polling creates race conditions and duplicates
        notification_key = f"user_notifications:{user_id}"
        ...
        # Fixes: Atomic delivery tracking prevents race conditions
    ...
    def mark_notification_read(self, user_id, notification_id):
        # Update read status without losing data
        # Fixes: Efficient status updates without data loss
        data_key = f"notification_data:{notification_id}"
        self.redis_cluster.hset(data_key, "read", "true")
```
...
class InAppNotificationClient {
    constructor(apiBaseUrl, authToken) {
        ...
        // Fixes: Device-specific delivery tracking
