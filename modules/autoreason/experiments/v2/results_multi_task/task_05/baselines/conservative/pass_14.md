...
### Team Distribution
- **US Team:** 8 engineers (4 senior, 4 mid-level)
- **EU Team:** 7 engineers (3 senior, 4 mid-level)
...
- Continue rotation ensuring senior/mid-level pairings
...
- Outgoing team posts handoff summary in #incident-handoffs
- Includes: active incidents, monitoring concerns, pending escalations

**Handoff summary template:**
```
🌍 [REGION] → [REGION] HANDOFF - [Date] [Time] UTC
...
[Incoming region] team: @[on-call] you have the watch
```
...
3. Detailed technical briefing including:
   - Current understanding of root cause
   - Steps attempted and results
   - Customer impact and communications sent
   - Next planned actions and timeline
4. Explicit acknowledgment from receiving team
...
- Incident Commander (IC) remains constant until resolution or explicit handoff
- If IC needs to hand off due to shift change, requires Engineering Manager approval
- New IC must be briefed by outgoing IC + Engineering Manager
- Customer communications maintained by single point of contact to ensure consistency

**Follow-the-Sun Coverage for Extended Incidents:**
- 8-hour maximum shifts for incident response to prevent fatigue
- Mandatory 4-hour break between shifts for same individual
- Detailed incident log maintained in shared document (updated every 30 minutes)
- Video recordings of technical briefings for complex issues

### Weekend and Holiday Coverage

**Weekend Coverage:**
- Reduced team: 1 primary + 1 secondary per region
- Engineering Manager on-call for escalations
- Automatic escalation timers reduced by 50% due to limited resources

**Holiday Coverage:**
- Advanced planning required 30 days ahead
- Volunteer-based with premium compensation (+50% hourly rate)
- Backup arrangements for each role
- Executive escalation paths adjusted for holiday schedules
